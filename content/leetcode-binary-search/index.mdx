...
```python
    def search(self, nums: List[int], target: int) -> int:
        ...
        while left <= right:
            ...
            # Discard left side of search space [left, mid, right]
            # Update search space, exclude mid: [mid+1, right]
            elif nums[mid] < target:
                left = mid + 1
            
            # Target is lower than mid:
            # Discard right side of search space [left, mid, right]
            # Update search space, exclude mid: [left, mid-1]
            else: 
                right = mid - 1
