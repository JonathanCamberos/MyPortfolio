...
```python
    def search(self, nums: List[int], target: int) -> int:
        ...
        # Binary Search 1: Target Search
        # Recursive if statement that checks a middle target,
        # and depending on result searches the left or right subspace.
        # Recursive binary search calls continue for O(log n) on
        # smaller and smaller range,
        # and creates a call stack of O(log n)
        ...
        #  Search 1: 
        # Target SeBinaryarch Search Space: '<='
        ...
        result = bs_recursive(0, len(nums) - 1)
...
```python
    def search(self, nums: List[int], target: int) -> int:
        ...
        # Binary Search 1: Target Search
        # Iterative if statement that checks a middle element,
        # and depending on result when compared to target,
        # searches the left or right subspace.
        # Iterative binary search checks continue for O(log n) on 
        # smaller and smaller range,
        # but since we are using pointers there is no call stack O(1)
...
```python
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        ...
        # Helper:
        # Revert 1D index back to original 2D coordinates (row, col)
        ...
        # Binary Search 1: 
        # Target Search Search Space: '<='
        # tc: binary search on flattened 2D matrix [0,(m*n)-1] O(log(m*n))
        while left <= right:
...
```python
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        ...
        # Target must lie within range of candidate row window [left, right] (if it exists)
...
```python
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        # Binary Search 2: Optimization Search
        # Iterative if statement that checks if middle element is valid
        # within some parameter check (in this case under h minimum hours),
        # and searches the left or right subspace 
        # and includes the middle element (if it was valid).
        # Iterative binary search checks continue for O(log n) on 
        # smaller and smaller range,
        # but since we are using pointers there is no call stack O(1)
        ...
        # Piles = non empty list of positive integers
        # h is at least greater than the number of piles: len(piles) <= h
        ...
        # Helper:
        # Finds the number of hours h needed for candidate k eating speed
        # to finish all of the piles of bananas
        # tc: pile iteration O(n)
        def hours_needed(speed):
            ...
            for pile in piles:
                ...
                # k = candidate eating speed
                # time for pile p = ceil(bananas in p / banana eating speed)
                # Round up time, cannot stop eating in middle of hour
                # For example: 
                # p = 3 bananas, k = 9 eating speed, 
                # will still take 1 full hour
                total_hours += math.ceil(pile / speed)

            # h hours needed for k eating speed for all piles
            return total_hours
        
        # Binary Search 2: 
        # Optimization Search Space: '<'
        # tc: binary search over k speeds O(log(max(piles)))
        while left < right:
            ...
            # Search left side of previous space, include mid: [left, mid]
            if required_hours <= h:
                right = mid
...
```python
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        # README.md
        # System design exercise regarding networking packets analogy
        ...
        # Greedy Time Search Optimization: 
        ...
        # Greedy Networking Start:
        #   - Start with the highest reasonable lower bound to speed up search efficiently
        #   - Different system perspectives for lower bound, at least 3
        #   - Use the networking analogy to determine the perspectives

        # Networking Analogy To Choose Heuristics:
        #   - Packets (piles)
        #   - Data size (bananas in pile)
        #   - Time Constraint (h)
        #   - Should respect at least two or all three (packets, data size, and time) in order to generate a good lower bound k
        ...
        # Must be lower bound and never overestimate = smallest packet
        # Close to real answer in order to minimize search time = average of total
        ...
        # Disregard other perspectives by bad choices:
        # 1. Average between all piles
        #   - Does not consider the time constraint, and basically assumes infinite time
        # 2. Max pile divided over h hours
        #   - Does not factor rest of piles, if max pile is much larger, we would waster k speed on the smaller piles
        ...
        # ------------------------
        # Heuristic 1: Throughput/Global Bandwidth
        #       - Total bananas being processed
        ...
        # ------------------------
        # Heuristic 2: Quick Packet Bursts
        #       - Smallest Pile Consideration With Fair Time Allocation
        #       - Speed based on smallest pile
        #       - To avoid having excessively large speed 
        # Part 1: Split Time Evenly By Fair time Sharing For Each Pile Constraint = Hour Per Pile Based Average, 
        # Part 2: Minimum Speed to Finish Smallest Pile within Split Even Time
        ...
        # ------------------------
        # Heuristic 3: Bottleneck Detection Prevention
        #       - Largest Pile Consideration With Unfair Time Allocation
        #       - Speed based on largest pile
        #       - Avoid having small speed that gets stuck on largest pile
        # Part 1: Biggest Pile Based Speed
        # Give Koko 1 hour per pile, except for the largest pile which will get the remaining time 
        ...
        # ------------
        # Greedy Starting Speed: 
        # We have 3 lower bounds from 3 different networking perspectives
        # Pick the highest lower bound between the 3 to avoid testing low speeds
        speed = max(speedCheck1, speedCheck2, speedCheck3)
