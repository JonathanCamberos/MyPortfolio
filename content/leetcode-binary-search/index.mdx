...
```python
    def findMedianSortedArrays(self, nums1, nums2):
        ...
        # Invariant:
        # After i steps, we have discarded i smallest values from the combined stream
        # get_min() always returns the next smallest remaining element

        # Helper function: simulate one step of merge
        def get_min():
            nonlocal p1, p2

            # Case 1: both pointers in bounds
            # Iterate over smaller value
            if p1 < m and p2 < n:
            ...
            # Case 2: One of the pointers is exhausted

            # nums2 exhausted: move nums1 
            elif p2 == n:
                ans = nums1[p1]
                p1 += 1

            # nums1 exhausted: move nums2
            else:
                ans = nums2[p2]
                p2 += 1
        ...
        # Note:
        # Simulate merge up to median index
        # For length k = m + n, median is:
        # If Odd: get k//2-th element
        # If Even: average (k//2 - 1)-th, k//2-th)

        # even length merged array
        if (m + n) % 2 == 0:

            # Discard stopping at (k//2 - 1) elements
            for _ in range((m + n) // 2 - 1):
                _ = get_min()

            # Grab (k//2-1) and (k//2) and get average
            return (get_min() + get_min()) / 2

        # odd length merged array
        else:

            # Discard stopping at k//2 elements
            for _ in range((m + n) // 2):
                _ = get_min()
            # Take next one as median
            return get_min()
```
|  Aspect  | Time Complexity | Space Complexity |  Time Remarks | Space Remarks |
| -------- | --------------- | ---------------- | ------------- |  ------------ |
...
```python
   def findMedianSortedArrays(self, nums1, nums2):
        ...
        # Precondition:
        # nums1 and nums2 are sorting in ascending order
        # 0 <= len(nums1), len(nums2) <= 10^6
        ...
        # Set nums1 as the smaller array
        ...
        while low <= high:
            ...
            # Partition nums1 at index partitionX
            partitionX = (low + high) // 2

            # We want the total number of elements in the left half to be
            # ()
