...
```python
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        # tc: binary search over 1 -> max(piles) speeds O(log(max(piles)))
        while left < right:
        ...
        # The space of valid speeds, [left, right], 
        # has been reduced to one speed
...
```python
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        # ------------
        # Greedy Starting Speed k:
        #   - Start k at highest lower bound between the 3 perspectives 
        ...
        # tc phase 1 doubling x2: exponential doubling O(log max(piles))
        # tc phase 2 iterating +1: linear scan O(max(piles))
        while (True):
            ...
            # Implies: this is the first valid k we encounter and is the min
            if smallKTotalTime <= h:
                return speed
            ...
            # Then: iterate by 1 until we find the first k valid speed
...
```python
   def findMedianSortedArrays(self, nums1, nums2):
        ...
        # Merged array partitions represents:
        # nums1: [1, 3 | 8]          => left max = 3, right min = 8 
        # nums2: [7 | 9, 10, 11]     => left max = 7, right mind = 7

        # The Final Partition Will Pass These Conditions:
        ...
        # The Final Partition Will Find Contain The Median:
        ...
        #
        #   1. Left num count >= Right num count
        #
        #   2. max left <= min right & max left <= min right:
        #
