...
```python
    def search(self, nums: List[int], target: int) -> int:
        ...
        # Invariant:
        # Subarray nums[left:right+1] contains the target if it exists
...
### Find the Bug: This House No Roof
```python
```
...
```python
    ...
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        # Invariant:
        # The correct k min speed is in the interval [left, right]
        ...
        while left < right:
            ...
            # No target check: finding min k

            # time is valid -> speed is valid
            # discard faster speed right half of [left, mid, right]
            # new search interval: [left, mid]
            if required_hours <= h:
                right = mid
            
            # time is invalid -> speed is invalid
            # discard slower speed left half of [left, mid, right]
            # new search interval: [mid + 1, right]
            else:
                left = mid + 1
        
        # Via invariant: the correct k min speed is in the interval [left, right]
...
```python
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        # first total time <= h, speed is guaranteed to be min due to greedy search strategy
        ...
        while (True):
            ...
            # totalTime is twice required time -> double speed k
            # new search interval: [speed*2, +inf]
            # time complexity: exponential time O(log max(piles))
            elif smallKTotalTime >= 2 * h:
                speed *= 2

            # totalTime close to required time -> increment speed by 1
            # new search interval: [speed+1, +inf]
            # time complexity: linear time O(max(piles))
            else:
                speed += 1
...
```python
    def findPeakElement(self, nums: List[int]) -> int:
        ...
        # Loop invariant:
        # At least one peak exists within [left, right]
        # At each step, we eliminate half of the range while preserving at least one peak
        while left < right:
            mid = (left + right) // 2

            # If mid is increasing towards the right, peak must lie to the right
            # because nums[mid] < nums[mid + 1]
        ...
        # At this point, left == right, and the loop invariant guarantees it's a peak
