...
### Solution 1: Two Phase Binary Search Row and Column - Binary Search/Searching
```python
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        ...
        while top <= bottom:
            mid_row = (top + bottom) // 2
            if matrix[mid_row][0] <= target <= matrix[mid_row][-1]:
                # Target must be in this row, break to search within it
                break
            elif matrix[mid_row][0] > target:
                bottom = mid_row - 1
            else:
...
### Solution 2: Flattened Matrix Binary Search - Binary Search/Searching
```python
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        
        # Note:
        # This solution treats the 2D matrix as a sorted 1D array by 
        # flattening the indices. It uses binary search on the 1D index space,
        # then converts indices back to 2D coordinates with the helper function.
        ...
        # time complexity: binary search on [0, m*n - 1] takes O(log(m*n))
