...
### Binary Search Application: Optimization Based on Target Upper Bound or Lower Bound Binary Search
...
Instead of directly searching for largest element less than or equal to target 
we search for the first element greater than the target and then simply 
...
```python
    class TimeMap:
        ...
        def get(self, key: str, timestamp: int) -> str:
            ...
            # grab list of tuples: (value, timestamp)
            values = self.store[key]

            # outer boundaries
            i = len(values) - 1
            
            # Reverse linear scan to grab
...
```python
    class TimeMap:
        ...
        def get(self, key: str, timestamp: int) -> str:
            ...
            # Note:
            # We cannot directly optimize for 'timestamp <= target' using '<=' in this binary search loop,
            # because that would complicate pointer movement and termination.
            # Instead, we optimize for the condition 'timestamp < target' (strictly less than).
            # This way, the search finds the index of the first timestamp greater than the target.
            # After the loop, we shift the pointer by one (left - 1) to get the largest timestamp <= target.
            ...
            # Binary search: find the first index with timestamp > query timestamp
            # time complexity: binary search over time stamps
            while left < right:
                
                mid = (left + right) // 2

                # If the timestamp at mid is less than or equal to the query timestamp,
                # then the current mid could still be a valid candidate,
