...
```python
    def search(self, nums: List[int], target: int) -> int:
        ...
        # Recursive binary search calls continue smaller range,
        # leading to a call stack of O(log n) (the height of a tree)
...
## Solution 2: Iterative Two Pointer Binary Search - Binary Search/Searching
```python
    def search(self, nums: List[int], target: int) -> int:
        
        # Binary Search:
        # Iterative if statement that checks a middle target,
        # and depending on result searches the left or right subspace.
        # Iterative binary search calls continue smaller range,
...
## Solution 1: Iterative Two Pointer Flattened Matrix by Index To Coordinates Binary Search - Binary Search/Searching
```python
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
...
## Solution 2: Outer Binary Search Over Row Ranges And Inner Binary Search Within Row Candidate - Binary Search/Searching
```python
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
...
```python
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        # len(piles) <= h, where h is the number of hours, find h
        ...
        # Calculate the hours needed for some candidate k eating speed
        def hours_needed(speed):
            ...
            for pile in piles:
                ...
                # Koko current has candidate k eating speed
                # For each pile koko will take ceil(pile/k) hours, 
                # as she cannot stop in the middle of an hour so she must round up
                # For example: 
                # for a pile with 3 bananas at 9 eating speed, she will still take 1 full hour
                total_hours += math.ceil(pile / speed)

            # result for k eating speed
            return total_hours
