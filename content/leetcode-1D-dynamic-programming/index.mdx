...
### Solution 1: DP with Rolling Max Min - 1D Dynamic Programming/Track Multiple States per Index (Max Min Tracking)
```python
...
### Solution 2: DP Array Version - 1D Dynamic Programming/Track Multiple States per Index (Max Min Tracking)
```python
    def maxProduct(self, nums: List[int]) -> int:
        n = len(nums)
        if n == 0:
...
### Solution 3: optimal dfs check why - 1D Dynamic Programming/Sequential Segment Choice Validation
```python
    def wordBreak(self, s: str, wordDict: List[str]) -> bool:
        cache = {}
        def dfs(i):
