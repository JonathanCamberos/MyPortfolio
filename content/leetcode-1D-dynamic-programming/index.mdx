...
```python
    def climbStairs(self, n: int) -> int:
        ...
        # 2. Boundary Check -> Reached N
        # 3. Boundary Check -> Overshot
        # 4. Build -> Save ways from i to N
        ...
        def dfs(i):
            ...
            # Build: sum of paths from next sub problems 
            memo[i] = left + right

            # Backtrack: return i to nth result
            return memo[i]
...
### Solution 2: 0 to N Recursive with Memoization Top Down - 1D Dynamic Programming/Sequential State Transition
```python
    def climbStairs(self, n: int) -> int:
        ...
        # 1. Memo Check -> computed previously
        # 2. Boundary Check -> Reached Bottom 0
        # 3. Boundary Check -> Reached Bottom 1
        # 4. Boundary Check -> Reached Bottom 2 
        ...
        def dfs(i):
            ...
            # Boundary Check -> Reached Bottom 1
            if i == 1:
                return 1

            # Boundary Check -> Reached Bottom 2
            if i == 2:
                return 2

            # Explore Choices -> arrived from 1 or 2 steps
            left = dfs(i - 1)
            right = dfs(i - 2)

            # Build: sum of paths from previous sub problems
            memo[i] = left + right

            # Backtrack: return 0 to ith result
            return memo[i]
...
### Solution 3: Iterative Bottom Up Optimal - 1D Dynamic Programming/Sequential State Transition
```python
    def climbStairs(self, n: int) -> int:
        
        # Note:
        # Iterative Bottom Up Tabulation
        # Build dp array for number of ways to climb from 0 to ith
        # 1. Base Set Up -> initial cases of dp[1], dp[2]
        # 2. Iterate Explore 3 to nth -> dp[i] = dp[i-1] + dp[i-2]
        # 3. Build -> fill array iteratively
        # Result: dp[n] is number of ways to climb to nth step

        # Initialize case: for n=1 assign n=2 will fail
        if n == 1:
            return 1
...
### Solution 4: Iterative Bottom Up Optimal - 1D Dynamic Programming/Sequential State Transition
```python
    def climbStairs(self, n: int) -> int:
        
        # Note:
        # Iterative Bottom Up Optimal
        # Build dp variables for number of ways to climb from 0 to ith
        # 1. Variable Set Up -> initial cases of prev2, prev1
        # 2. Iterate Explore 3 to nth -> dp[i] = prev2 + prev1
        # 3. Build -> fill ans iteratively
        # Result: dp[n] is number of ways to climb to nth step
