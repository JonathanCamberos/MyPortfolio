...
what the state of an index represents:

Or just use a mental trick: 
If we do size (N) we need dp[n-1] as final answer.
If we do size (N+1) we need dp[n] as final answer.

1. Prefix View (N+1)

dp[i] is defined as the answer for the first i elements
(e.g., houses[0..i-1] or steps[0..i-1])
...
dp[i] is defined as the answer for element i itself (houses[i] or step i)
dp[0] = answer for 1 element (first element)
dp[n-1] = answer for last element
Size must be dp[n]

House Robber:
(N) Natural + (N+1) Shifted:
...
```python
    def climbStairs(n: int) -> int:
        ...
        # Without this, dp[1] would cause IndexError
        if n == 1:
            return 1
...
```python
    def climbStairs(self, n: int) -> int:
        ...
        # 2. Initialize Array -> ways from 0 to i, size n+1 (n is indexed 1)
        # 3. Base Set Up -> 1 and 2
        # 4. Iterate -> 3 to n
        # 5. Explore -> grab num of ways from 0 to -2 or -1 steps
        # 6. Build -> sum paths from -2 and -1 steps to current step
        ...
        # Initialize array -> ways from 0 to i, size n+1 (n is indexed 1)
        dp = [0] * (n + 1)
...
### Solution 4: 0 to i Iterative Bottom Up Variables - 1D Dynamic Programming/Sequential State Transition
```python
    def climbStairs(self, n: int) -> int:
...
### Solution 1: i to N Recursive with Explicit Memoization Top Down - 1D Dynamic Programming/Min Max Path Sum in Linear Sequence
```python
    def minCostClimbingStairs(self, cost: List[int]) -> int:
        ...
        # 2. Boundary Check -> base cases at step 0 and step 1
        # 3. Explore Choices -> to reach i, come from i-1 or i-2
        # 4. Build -> get min path between -1 and -2 steps + cost at current step
        # 5. Memo Return -> cost to reach i, calculated once
        # Result: min cost to reach top step at n from 0 or 1
        ...
        def dfs(i):
            ...
            # Memoized check -> avoid recomputation
            if i in memo:
                return memo[i]

            # Boundary Cases -> directly return base costs
            if i == 0:
                return cost[0]
            if i == 1:
                return cost[1]

            # Explore Choices -> cost to reach i
            left = dfs(i-1)
            right = dfs(i-2)
        ...
        # reach n by continuing from n-1 or n-2
...
### Solution 2: 0 to i Iterative Bottom Up Array - 1D Dynamic Programming/Min Max Path Sum in Linear Sequence
```python
    def minCostClimbingStairs(self, cost: List[int]) -> int:
        ...
        # 2. Initialize Array -> min cost to reach + continue from i, size n (n indexed 0)
        # 3. Iterate -> 2 to n-1
        # 4. Explore choices -> dp[i] = cost[i] + min(dp[i-1], dp[i-2])
        # 5. Build -> fill dp array sequentially
        # Result: min cost to continue from step n-1 or n-2 (to reach step n)

        n = len(cost)

        # Initialize Array -> min cost to reach + continue from i, size n (n indexed 0)
        dp = [0] * n
        ...
        for i in range(2, n):
            ...
            # Explore Choices -> cost to reach and continue from -1 or -2 steps
            left = dp[i-2]
            right = dp[i-1]

            # Build -> cost to continue from step i  + min between +1 or +2 cost
            dp[i] = cost[i] + min(left, right)

        # Build -> cost to continue from n-2 or n-1 step (both of which can reach nth)
...
### Solution 3: 0 to i Iterative Bottom Up Variables - 1D Dynamic Programming/Min Max Path Sum in Linear Sequence
```python
    def minCostClimbingStairs(self, cost: List[int]) -> int:
        ...
        # 2. Iterate -> 2 to n-1
        # 3. Explore -> cost for continue from -2 or -1 steps
        # 4. Build -> cost to continue from curr step + min between -2 or -1 steps
        # 3. Slide window -> update prev2, prev1
        # Result: min cost to continue from n-2 or n-1 steps (to reach n)
...
```python
    def tribonacci(self, n: int) -> int:
        ...
        # Top Down DP (memoized recursion)
        # 1. Memo Check -> computed previously
        # 2. Boundary Check -> reached base case
        # 3. Explore Choices -> grab num of ways from i+1, i+2, i+3
        # 4. Build -> sum paths from three previous steps
        # 5. Memo Return -> return ways from i to nth step, calculated once
        ...
        def dfs(i):
            ...
            # Boundary Check -> base cases
            if i == 0:
                return 0
            if i == 1 or i == 2:
                return 1

            # Explore Choices -> sum of previous three steps
            left = dfs(i-1)
            mid = dfs(i-2)
            right = dfs(i-3)

            # Build -> sum paths
            memo[i] = left + mid + right
...
```python
    def tribonacci(self, n: int) -> int:
        ...
        # Top Down DP (memoized recursion)
        # 1. Memo Check -> computed previously
        # 2. Boundary Check -> base cases 0,1,2
        # 3. Explore Choices -> sum of previous three
        # 4. Build -> T_i = sum(T_{i-1}, T_{i-2}, T_{i-3})
        # 5. Memo Return -> return T_i, calculated once
...
```python
    def tribonacci(self, n: int) -> int:
        ...
        # Note:
        # Iterative Bottom Up Variables
        # 1. Catch Initialize -> n = 0,1,2 base cases
        # 2. Base Set Up -> T0, T1, T2
        # 3. Iterate -> 3 to n
        # 4. Explore + Build -> T[i] = sum of previous three
        # 5. Slide Window -> update variables
        # Result: T_n in Tribonacci sequence

        # Catch Initialize -> base cases
        if n == 0:
            return 0
        if n == 1 or n == 2:
            return 1

        # Base Set Up -> previous three values
        T0, T1, T2 = 0, 1, 1

        # Iterate -> compute from 3 to n
        for i in range(3, n + 1):

            # Explore + Build -> sum of previous three
            curr = T0 + T1 + T2

            # Slide Window -> shift variables
            T0, T1, T2 = T1, T2, curr

        # Result -> nth Tribonacci number
        res = T2
        return res
```
...
### Solution 1: i to N Recursive with Explicit Memoization Top Down - 1D Dynamic Programming/Choose or Skip Non Adjacent
```python
    def rob(self, nums: List[int]) -> int:
        ...
        # max loot from house start to ith house
        memo = {}
        ...
        def dfs(i):
            ...
            # Boundary Check -> max between first house
            if i == 0:
                return nums[0]

            # Boundary Check -> max between first and second house
            if i == 1:
...
### Solution 2: 0 to i Iterative Bottom Up Array - 1D Dynamic Programming/Choose or Skip Non Adjacent
```python
    def rob(self, nums: List[int]) -> int:
        
        # Note:
        # Bottom Up Tabulation
        # 1. Catch Initialize -> case 0
        # 2. Initialize Array -> max loot for 0 to ith house, size n (n indexed 0)
        # 3. Base Set Up -> 0 and 1
        # 4. Explore -> 2 to n-1
        # 5. Explore choices -> dp[i] = max(dp[i-1], nums[i] + dp[i-2])
        # 6. Build -> fill dp array sequentially
        # Result: max loot from 0 to nth house
        ...
        # Initialize Array -> max loot for 0 to ith house, size n (n indexed 0)
        dp = [0] * n
...
### Solution 3: 0 to i Iterative Bottom Up Variables - 1D Dynamic Programming/Choose or Skip Non Adjacent
```python
    def rob(self, nums: List[int]) -> int:
...
### Solution 1: i to N Recursive with Memoization Top Down - 1D Dynamic Programming/Circular Choose or Skip Non Adjacent
```python
    def rob(self, nums: List[int]) -> int:
...
### Solution 2: 0 to i Iterative Bottom Up Array - 1D Dynamic Programming/Circular Choose or Skip Non Adjacent
```python
    def rob(self, nums: List[int]) -> int:
        ...
        # Catch Initialize -> amount 1
        if n == 1:
            return nums[0]

        # max loot from subarray start -> end
        def rob_range(start, end):

            # Boundary Set -> number of houses in subarray
            numHouses = end - start + 1

            # Boundary Check -> single house, max loot is robbing
            if numHouses == 1:
                return nums[start]

            # Initialize Array -> max loot from start to ith house, size numHouse (0 indexed)
            dp = [0] * numHouses
            ...
            # Memo return -> max loot at ith, calculated once
            return dp[numHouses-1]


        # Break circular constraint -> exclude last house, exclude first house
        res1 = rob_range(0, n-2)
        res2 = rob_range(1, n-1)

        # Result -> max loot between ignore either first or last house
        res = max(res1, res2)
...
### Solution 3: 0 to i Iterative Bottom Up Variables - 1D Dynamic Programming/Circular Choose or Skip Non Adjacent
```python
    def rob(self, nums: List[int]) -> int:
...
### Solution 1: 0 to i Recursive with Memoization Top Down - 1D Dynamic Programming/Sequential State Transition
```python
    def numDecodings(self, s: str) -> int:
        ...
        # Top Down DP (recursive with memoization)
        # 1. Memo Check -> avoid recomputation
        # 2. Boundary Check -> base cases for empty string and first char
        # 3. Explore Choices -> to reach i, decode single or double digit ending at i
        # 4. Build -> sum of decoding ways from valid previous states
        # 5. Memo Return -> total decode ways for prefix [0..i]
        ...
        def dfs(i):
            ...
            # Boundary Check -> empty "" has 1 way
            if i < 0:
                return 1

            # Boundary Check -> single char has 1 way, cannot be '0'
            if i == 0:
                return 1 if s[0] != '0' else 0

            count = 0

            # Explore Choices -> decode ignoring single digit
            if s[i] != '0':
                count += dfs(i-1)

            # Explore Choices -> decode ignoring double digit
            two_digit = int(s[i-1:i+1])
            if 10 <= two_digit <= 26:
                count += dfs(i-2)

            # Memo Return -> count for [0, i]
            memo[i] = count
            return count

        # Decode ways for full string (0..n-1)
        return dfs(n-1)
```
...
```python
    def numDecodings(self, s: str) -> int:
        ...
        # Note:
        # Bottom Up DP (array version, aligned with recursive definition)
        # dp[i] = number of ways to decode prefix s[0..i]
        # 1. Initialize Catch -> empty string
        # 2. Base Set Up -> dp[0] for first character
        # 3. Iterate -> compute dp[i] for i = 1..n-1
        # 4. Explore Choices -> single-digit and double-digit decoding
        # 5. Build -> sum valid decodings from previous states
        # 6. Result -> dp[n-1] contains total decode ways for full string

        n = len(s)
        if not s:
            return 0

        # Initialize Array -> number of ways to decode prefix s[0..i]
        dp = [0] * n

        # Base Set Up -> first char
        dp[0] = 1 if s[0] != '0' else 0

        # Iterate -> fill dp from left to right
        for i in range(1, n):

            # Explore Choices -> single digit (s[i])
            if s[i] != '0':
                dp[i] += dp[i-1]

            # Explore Choices -> double digit (s[i-1:i+1])
            two_digit = int(s[i-1:i+1])
            if 10 <= two_digit <= 26:
                # Add ways from two steps back if i >= 2, otherwise 1
                dp[i] += dp[i-2] if i >= 2 else 1

        # Result -> total decoding ways for full string s[0..n-1]
        res = dp[n-1]
...
```python
    def numDecodings(self, s: str) -> int:
        ...
        # 4. Result -> prev1 contains total decode ways for full string

        # Invalid check -> empty string
        if not s:
            return 0

        n = len(s)

        # Base cases -> first character
        prev2 = 0  # dp[-2], used for two-digit calculation at i = 1
        prev1 = 1 if s[0] != '0' else 0  # dp[0], ways to decode s[0]

        # Iterate -> from second character to last
        for i in range(1, n):
            ...
            # Slide Window -> update previous two states
            prev2, prev1 = prev1, curr
...
### Solution 1: Recursive with Memoization Top Down - 1D Dynamic Programming/Min Max with Choices (Unbounded Knapsack style)
```python
    def coinChange(self, coins: List[int], amount: int) -> int:

        # Note:
        # Top down (recursive with memoization)
        # 1. Memo Check -> recomputation
        # 2. Boundary Check -> overshoot or exact match
        # 3. Explore Choices -> recursively subtract each coin
        # 4. Build -> take min coins among all valid choices
        # 5. Memo Return -> return result for current remaining amount
        ...
        def dfs(remaining) -> int:
            ...
            for coin in coins:
                ...
                # Explore -> subtract coin value from remaining to check if valid combo exists
                left = dfs(remaining - coin)

                # Explore -> non -1 means valid number of coins)
                if left >= 0:

                    # Build -> min among valid choices
                    min_coins = min(min_coins, left + 1)

            # Memo Result -> store result for remaining amount
...
### Solution 2: Iterative Bottom Up Array - 1D Dynamic Programming/Min Max with Choices (Unbounded Knapsack style)
```python
    def coinChange(self, coins: List[int], amount: int) -> int:
        
        # Note:
        # Iterative Bottom Up Array
        # 1. Catch Initialize -> for amount=0
        # 2. Base Set Up -> 0
...
### Solution 3: BFS Level Order Search - 1D Dynamic Programming/Min Max with Choices (Unbounded Knapsack style)
```python
    def coinChange(self, coins: List[int], amount: int) -> int:
