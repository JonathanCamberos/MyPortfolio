...
### Solution 4: Forward Backward Scan - 1D Dynamic Programming/Opposite Ends Iterative (Direct N) Rolling State Variables Bottom Up
```python
    def maxProduct(self, nums: List[int]) -> int:
...
## 300. Longest Increasing Subsequence ::4:: - Medium
...
### Solution 3: Binary Search Patience Sorting - 1D Dynamic Programming/Subsequence Optimization Constrained
```python
    def lengthOfLIS(self, nums: List[int]) -> int:
...
### Solution 4: Binary Search - 1D Dynamic Programming/Subsequence Optimization Constrained
```python
    def lengthOfLIS(self, nums: List[int]) -> int:
