...
```python
    def climbStairs(self, n: int) -> int:
        ...
        # State Definition:
        # Let dfs(i) represent the number of ways to reach step n
        # starting from step i.

        # Recurrence Relation:
        # From step i, you can:
        #   - take 1 step -> go to i + 1
        #   - take 2 steps -> go to i + 2
        # Therefore:
        #   dfs(i) = dfs(i + 1) + dfs(i + 2)

        # Base Cases (Padded Boundaries):
        #   - If i == n: reached the top -> 1 valid way
        #   - If i > n: overshot -> 0 valid ways
        ...
        # Top-Down Traversal:
        # We start at step 0 and recursively move forward toward n.
        # Results propagate backward through the call stack.
        # This is DFS over a linear state space with caching.

        # State Space (1D DP Array Conceptually):
        #   Nodes: i where 0 <= i <= n
        #   Edges: i -> i+1, i -> i+2
        # This forms a directed acyclic graph (DAG).
        # We are counting paths from node 0 to node n.

        # Memoization: avoid recomputation of overlapping sub problems
        ...
        def dfs(i):
            ...
            # Base Case: Overshot
            # Check: i > n, we've exceeded to top step
            # tc: O(1)
            if i > n:
                return 0

            # Base Case: Target
            # Check: i == n, we've reached the top step
            # tc: O(1)
            if i == n:
                return 1

            # Recurrence Relation:
            # Build from future states: take 1 and 2 steps
            # dfs(i) = dfs(i+1) + dfs(i+2)
            # tc: O(1) per call, overall O(n) due to memoization
            memo[i] = dfs(i + 1) + dfs(i + 2)

            # Return:
            # Number of ways for step i
            # tc: O(1)
            return memo[i]

        # Initial Call:
        # Number of ways starting from step 0
        # tc: O(1)
        res = dfs(0)
...
```python
    def climbStairs(self, n: int) -> int:
        ...
        #   1. Overlapping subproblems
        #   2. Optimal substructure
        # Bottom-Up DP builds solutions iteratively from the smallest subproblems
        # to the target, eliminating recursion entirely.

        # Three DP Perspectives:
        # 1. i -> N (Top-Down Recursive with Memoization)
        # 2. 0 -> i (Top-Down Recursive / 0 → i)
        # 3. 0 -> i (Bottom-Up Iterative / Tabulation)
        # This solution uses 0 → i (Bottom-Up Iterative Tabulation)

        # State Definition:
        # dp[i] represents the number of ways to reach step i from step 0.

        # Base Cases (Padded Boundaries):
        #   - dp[0] = 1 -> 1 way to stay at ground level
        #   - dp[1] = 1 -> 1 way to reach first step
        # Padding simplifies the iterative logic, avoiding index errors.

        # Array Size:
        # if n = 5, the steps are [0, 1, 2, 3, 4, 5] => 6 elements => n + 1.
        ...
        # Iterate Build (Bottom-Up Traversal):
        # Start from step 2 up to n
        # For each step i, build dp[i] from dp[i-1] and dp[i-2]
...
```python
    def minCostClimbingStairs(self, cost: List[int]) -> int:
        ...
        #   1. Overlapping subproblems
        #   2. Optimal substructure
        # Top-Down DP computes the solution recursively and caches results
        # to avoid recomputation.
        ...
        # 2. 0 -> i (Bottom-Up Recursive)
        # 3. 0 → i (Bottom-Up Iterative / Tabulation)
        # This solution uses i → N (Top-Down Recursive with Memoization)

        # Problem Setup:
        # Let dfs(i) represent the minimum cost to reach step i
        # starting from step 0, considering cost array indexing (cost[i-1]).

        # Recurrence Relation:
        # Minimum cost to reach step i is:
        # cost to step i-1 + min(cost to reach i-1, cost to reach i-2)
            
        # sc: setting up search space rows [0, n] O(1)
        n = len(cost)

        # Memoization: avoid recomputation of overlapping subproblems
        # sc: O(n) for memo dictionary
        memo = {}
        ...
        def dfs(i):
            ...
            # Memo Check:
            # If step i has been computed previously, return it
            # tc: O(1)
            if i in memo:
                return memo[i]

            # Base Case: Ground Step
            # i == 0 -> no cost to stay at ground
            # tc: O(1)
            if i == 0:
                return 0

            # Base Case: First Step
            # i == 1 -> cost to reach first step is cost[0]
            # tc: O(1)
            if i == 1:
                return cost[0]

            # Build From Previous:
            # Number of ways to reach step i is cost to reach step i + min(cost between previous two steps)
