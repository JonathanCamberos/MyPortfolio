...
description: "designing data intensive applications: pg 16/544"
...
> First four chapters: fundamental ideas that apply to all data systems,
...
- Explain approaches to achieve these goals.

2. **Chapter 2:**

- Compare data models and query languages in different situations.

3. **Chapter 3:**

- Internals of storage engines.
- How database store data on disk.
- How different storage engines are optimized for different workloads.
- Choosing the right has large effect on performance.

4. **Chapter 4**: 

- Compare data encoding and serialization formats 
- Examine in environment where app requirements change and schemas need to adapt over time.

## Chapter 1: Reliable, Scalable, and Maintainable Applications

### Intro

> The Internet was done so well that most people think of it like a natural resource
...
the most issues, as opposed to compute intensive.
...
These building blocks have established and well built that most 'sane' engineers 
...
> performance and implementations under the umbrella term data systems
> as each of those act as building blocks for the larger 'data systems'
> The API layer hides those implementation details from clients
> allowing use to create a new special purpose data system 
> from smaller, general purpose.

New tools for data storage and processing continue emerging leading to trouble
as traditional categories fade and boundaries blur.
...
> Abstracting single data system with a API wrapper

APIs hide implementation details from clients, becoming a 
...
> desired level of performance, even in the face of adversity, that being 
> either hardware faults, software faults, human error, etc..
...
> It is in our best interest to trigger faults deliberately to ensure faults are handled
> gracefully when they occur naturally.

Basic expectations for when one says software is reliable can include:
...
What causes reliability to break is then the things that can go wrong.
A fault is our system design wrapper word for all the things that can go wrong.
Thus, a system that anticipates faults is fault tolerant or resilient.
Whereas a failure is different.

In the context of a system, a failure is when the system stops providing the required service
to the user. 
...
In fault-tolerant systems, we encourage and increase the rate of faults by triggering them 
deliberately to test our tolerance.
Many critical bugs are actually due to poor error handling and thus by continuously testing
we ensure faults will be handled correctly when they occur naturally
...
The first solution is to add redundancy to individual hardware components 
with RAID configuration, add dual power supplies to servers, 
...
Thus, there has been a shift towards systems that can tolerate the loss of entire
machine by using software fault-tolerance techniques in addition to hardware redundancy.
...
Hence why this shift benefits applications that require high availability.


#### Software Errors

> Software faults are harder to anticipate because they are 'non-random' across nodes.
> They also tend to cause more system failures than 'random' hardware faults (e.g., MTTF).
> The are 'non-random' as they might fault upon a bad input.
...
There is one one quick solution to faults in software.
But lots of small things can help:
...
Human configuration and touch is around the entire system making it hard
to anticipate.
But lots of small things can help:
...
We make the system as fault-tolerant and anticipate errors in order
to account for future human errors.
...
in the future.
Increased load is a common reason for future degradation. 
Jumping from 10,000 concurrent to 100,000 or 1 million to 10 million 
will require the system to process much larger volumes of data than before.
...
1. Post Tweet
A user can publish a new message to their followers. 
...
So the issue lies not with tweet volume, but with fan out from tweeter to follower.
Each user follows many people, and each user is followed by many people.

This may seem daunting at first, but there are two main ways
of implementing these two operations


##### 1. SQL Merge: Fan Out on Read

Idea: Do almost no extra work when someone posts a tweet. Just insert it into
the global tweets table.

When someone posts via PostTweet():
Insert into global tweets table

When uses requests HomeTimeline():
Look up list of people they follow, fetch tweets for each of those users,
merge them and sorted by timestamp

Work: Happens at read time (query + merge)

Optimized for: When writes are frequent but reads are infrequent

```sql
-- Fan-Out on Read: Build users home timeline on demand
...
Idea: Keep a precomputed 'mailbox' (cache) of tweets for each user.

When someone posts via PostTweet():
Find all their followers, insert that tweet into each follower's cached home timeline.

When uses requests HomeTimeline():
Just return their cached timeline, no merging required

Work: Happens at write time (copy tweet into many many caches)

Optimized for: When reads are more common than writes
...
##### Twitter Scaling Fan Out: Solution 1, then Solution 2 Due To Reads() > Writes()
...
##### Celebrity Fan Out Solution: Celebrity Tweet Merge + Cache Timeline Hybrid
...
##### Performance As a Distribution: p50, p95, p99, p999 and client load generation
...
High percentiles become especially in backend services that are called multiple 
times as part of serving a single end-user request.

Even if you make the calls in parallel, the client response will still wait on the 
slowest of the parallel calls.

Thus, even if a small percentage of backend calls are slow, the chance of getting
a slow call increases if a end-user request requires multiple backend calls,
leading to a higher proportion of end-user requests being slow, an effect 
known as tail latency amplification

- Def: Tail Latency Application: when a single client call leads to multiple parallel calls,
and must wait on the slowest parallel response, increasing the chance of a high percentile
slow call

In order to add response time percentiles to monitoring dashboards for your backend
services, you need to efficiently calculate them on an ongoing basis.

Naive Implementation: Keep a rolling window of response times of requests in the last
10 minutes, and every minute calculate the median and various percentiles.

There are actually algorithms specific for good approximation of percentiles 
at minimal CPU and memory cost

A common mistake is to average the percentile of two different machines, which would
be mathematically meaningless. A percentile depends on the distribution of all combined
requests, not the 95th or 50th percentile of each machine individually.
Machine A might have fast requests while machine B has slow ones and averaging
those separately losses the true combined distribution.

Forward Decay: streaming algorithm for approximating metrics like percentils
...
t-digest: Compact data structure for approximating percentiles and very accurate for
...
HdrHistogram: High Dynamic Range Histogram. Stores response time counts in a histogram.
