...
{/* def: topic=API id=2 perspective=Data Systems diagramId=simple-architecture */}
...
{/* def: topic=Throughput id=2 perspective=Performance distribution diagramId=high-percentile */}
Throughput would be the number of records we can process per second, or the total time
to complete a job on a dataset of a certain size
{/* end */}

However, in online systems, the response time is usually more important

{/* def: topic=Response Time id=2 perspective=performance distribution diagramId=high-percentile */}
