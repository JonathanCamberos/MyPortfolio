...
Use: Good when writes are frequent but reads are infrequent

##### Fan-Out on Read() Pseudocode
```sql
-- Fan-Out on Read: Build users home timeline on demand

    -- grab tweets and author info for everyone user follows from global tweet table
    SELECT 
        tweets.*, -- all tweets fields (id, text, timestamp, etc.)
        users.*   -- all user fields for tweet's author (name, username, etc.)
    FROM tweets

    -- join information about user, matching tweeter to tweet
    JOIN users 
