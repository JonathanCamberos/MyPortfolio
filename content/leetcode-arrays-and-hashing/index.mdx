...
This would lead to O(3n)

Lets see how we can do it in O(n):
...
### Brute Force (multiplying all except self)
```python
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        res = [0] * len(nums) # result array to store the product
...
```python
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        # Compute postfix products and update res
        postfix = 1
