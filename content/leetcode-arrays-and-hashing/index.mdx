...
image: "../../public/blogs/hashmap.png"
...
### Find the Bug: (all pairs comparison)
```python
    def containsDuplicate(self, nums: List[int]) -> bool:
        ...
        for i in nums:
            ...
            for j in nums: 
                ...
                # INCORRECT: iterating over elements of a collection
                # will lead to comparing element to itself at least once 
                #  vs index iteration
                if i == j 
                    return True
    ...
    def isAnagram(self, s: str, t: str) -> bool:
        ...
        for char1 in s:
            ...
            for char2 in t:
                ...
                # INCORRECT: does not take into account freq count for chars
                if char1 == char2:
                    return True
        ...
        return False
```
Above will not work as it does not take validate if the frequency count 
for characters is the same for both strings. 

Frequency count matching as well as occurrence is required for anagrams.
...
complement -> key -> index mapper for future elements.
...
|  Solution  | Time Complexity | Space Complexity | Time Remark | Space Remark |  
| ---------- | --------------- | ---------------- | ----------- | ------------ |
| Brute Force (Compare all pairs * Merge sort) | O(n<sup>2</sup> * k log k) | O(1) | For each pair of strings, we compare them leading to O(n<sup>2</sup>). Using a Hashmap to check if they are anagrams takes O(k). Leading to O(n<sup>2</sup> * k)  | Stores temporary intermediate groups in a list |
...
| Hashmap Tuple | O(n * k) | O(n * k) | Iterating over the Array takes O(n). Computing character count per string takes O(k). Leading to O(n * k) | Creates a Hashmap per string O(n) for k character key map per string O(k). Leading to O(n * k)|

### Brute Force (Compare all pairs * Merge sort)
Not shown in code 

Is extremely inefficient with O(n<sup>2</sup> * k log k). It compares each pair of strings
via merge sort for anagram grouping.
for inequality.
...
def longestConsecutive(self, nums: List[int]) -> int: 
    ...
    for i in numSet:
        ...
        # found start of new sequence (left most element)
        # time complexity: lookup operation takes constant O(1)
        if (i - 1) not in numSet: 

            # time complexity: iterate over potential n sequence O(n) + lookup operation constant O(1)
            currLen = 1
