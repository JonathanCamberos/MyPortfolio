...
### Find the Bug: Hashmap Key (Unhashable type)
```python
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
...
### Find the Bug: Hashmaps are not sorted
```python
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
...
### Solution 1: Hashmap of 26 to Tuple Key - Hashmap/Grouping by Criteria
```python
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
...
```python    
    def topKFrequent(self, nums: List[int], k:int) -> List[int]:
        ...
        # space complexity: worst case count for n unique elements O(n)
        count = defaultdict(int)
        ...
        # space complexity: worst case count array for n unique elements O(n)
        maxHeap = []

        # time complexity: worst case iterate over n unique elements O(n)
        for num, freq in count.items():

            # time complexity: push onto heap O(log n) leading to O(n log n) 
            heapq.heappush(maxHeap, (-freq, num))

        # space complexity: list of k elements O(k)
        result = []

        # time complexity: iterate for k elements O(k) pop top elements O(log n) leading to O(k log n)
        for _ in range(k):
...
| Building MaxHeap | O(n log n) | O(n) | Push n unique elements O(n) using bubbleUp O(log n) leading to O(n log n) | Memory allocation for array representing heap O(n) |
| Extracting K elements | O(k log n) | O(k) | Pop k elements O(k) using bubbleDown O(log n) leading to O(k log n) | Memory allocation to store k elements O(k) |
...
```python    
    def topKFrequent(self, nums: List[int], k:int) -> List[int]:
        ...
        for num, freq in count.items(): 
            ...
            # if heap grows to size k + 1, pop smallest element, heap will be back to size k
            if len(minHeap) > k:

                # time complexity: # O(log k)
                heapq.heappop(minHeap) 
        
        # minHeap is holding top k elements, return all elements
...
| -------- | --------------- | ---------------- | ------------- |  ------------ |
| Counting Frequencies | O(n) | O(n) | iterate trough input array of n length O(n) | Memory allocation for frequency count for n unique elements O(n) |
| Building MinHeap | O(n log k) | O(k) | Iterates over list of n length O(n) and performs bubbleUp on heap with height k O(log k), leading to O(n log k) | Memory allocation to represent heap of k length O(k) |
| Extracting top k | O(k) | O(k) | Iterate over heap for k elements O(k) | Memory allocation to store top k elements O(k) |
| Overall | O(n log n) | O(n) | Worst case iterating over count for n unique elements O(n) and pushing values to heap to track n values O(log n), leading to O(n log n) | Memory allocation to track in worst case n unique element counts O(n) |


### Solution 3: High to Low K Quickselect - Hashmap/Algorithm
```python
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
    
        # Note: Quickselect is a modified quicksort
        # Once the k-th smallest/largest element is placed in its correct position, elements
        # to the left and right are guaranteed to be smaller/larger
        # This avoids sorting the entire array and isolates the elements we need

        # In-place partition of array based on pivot value
        def partitionSection(left, right, pivotElemIndex):
            
            # Frequency of pivot element
            pivotElemFreq = frequency[unique[pivotElemIndex]]

            # Move pivot to end
            # tuple unpacking in python, allows to swap two variables in a single line
            unique[pivotElemIndex], unique[right] = unique[right], unique[pivotElemIndex]
        ...
        def quickselectHelper(left, right, kSmallest):
            ...
            pivotElemIndex = random.randint(left, right)

            # Partition the array
            pivotElemIndex = partitionPivot(left, right, pivotElemIndex)
            
            # Base Case: k smallest element placed in correct index
            # Grab everything to the left of it
            if kSmallest == pivotElemIndex:
                return 
            
            # Recursively selects next partition pivot
            # Ideally we would partition starting in middle
            # and recursively partition the left and right side
            # until one of the pivotElemIndexes is kSmallest
            elif kSmallest < pivotElemIndex:
                quickselectHelper(left, pivotElemIndex - 1, kSmallest)
            else:
                quickselectHelper(pivotElemIndex + 1, right, kSmallest)
        ...
        # For high-to-low sorting, k-th smallest corresponds to index k - 1
        # 4 elements top 2
        # 2 - 1 = 1 marker index
        # top 2 index to the left of marker index [0, 1]
        # 0 1 2 3    
        quickselectHelper(0, n - 1, k - 1)

        # 4 elements top 2 [0, 1]
        # [:2]
        # overall: time complexity
        # overall: space complexity 
        return unique[:k]
...
### Solution 4: Low to High K Quickselect - Hashmap/Algorithm
```python
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        def quickselectHelper(left, right, kSmallest):
            ...
            # Base Case: k smallest element placed in correct index
            # Grab everything to the right of it
            if kSmallest == pivotElemIndex:
                return
        ...
        # time complexity: iterate over list of n length O(n)        frequency = defaultdict(int)
        frequency = defaultdict(int)
        for num in nums:
            frequency[num] += 1
        ...
        # 4 elements top 2
        # 4 - 2 = 2 marker index
        # top 2 index to the right of marker index [3, 4]
        # 0 1 2 3       
        quickselectHelper(0, n - 1, n - k)

        # 4 elements top 2: [2,3]
        # [2:]
        # overall: time complexity
        # overall: space complexity
        return unique[n - k:]
...
```python
    def topKFrequent(self, nums: List[int], k:int) -> List[int]:
        ...
        # below is saying [] list, with inner empty list count [] len(numBuckets) 
...
### Solution 1: HashMap Boundaries - HashMap/Representation
```python
    def longestConsecutive(nums: List[int]) -> int:
...
|  Aspect  | Time Complexity | Space Complexity |  Time Remarks | Space Remarks |
| -------- | --------------- | ---------------- | ------------- |  ------------ |
| Hashset | O(n) | O(n) | Converting list to hashset uses insert operation O(1) for n linear O(n) | Memory allocation for list o n length O(n) |
| Outer Loop | O(n) | O(1) | Iterating over hashset of n length O(n) | No additional memory allocation for iteration |
