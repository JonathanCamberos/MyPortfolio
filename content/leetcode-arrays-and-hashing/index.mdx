...
### Find the Bug: Hashmap Item Order (Hashmaps are not sorted)
```python
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
...
### Solution 3: High Low QuickSelect BinarySearch - Hashmap/Algorithm
```python
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        def quickselectHelper(left, right, kSmallest):
            ...
            # Base case: the section contains only one element
            if left == right:
                return
            ...
            # Partition the array
            sortedPivotElemIndex = partitionSection(left, right, pivotElemIndex)
            
            # Base Case: k smallest element placed in correct index
            # Grab everything to the left of it
            if kSmallest == sortedPivotElemIndex:
                return 
            
            # Binary Search Modification:
            # Selects next partition pivot
            # Ideally we would only QuickSelect partitioned section (left or right)
            # where kSmallest belongs to
            elif kSmallest < sortedPivotElemIndex:
                quickselectHelper(left, sortedPivotElemIndex - 1, kSmallest)
            else:
                quickselectHelper(sortedPivotElemIndex + 1, right, kSmallest)
...
### Solution 4: Low High QuickSelect BinarySearch - Hashmap/Algorithm
```python
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
