...
Calculation for O(n<sup>2</sup>) is found [here](/blogs/common-formulas#brute-case-double-for-loop).

Space Complexity: O(1)

No additional data strcutures are used. Only the given variables are maintained.
...
We iterate over the list once. Time Complexity is O(n).

Space Complexity: O(n)

We traverse the list, adding a count for every element we've seen. If we check an 
element in the hashmap which has >= 1 count, it means we've seen that element previously and 
have found a duplicate

Worst case is a list with no duplicates in which case we store O(n) keys with values of 1. Space Complexity is O(n).
...
We traverse the list, adding an element into a set if the element is not already present. 

Worse case is a list with no duplicates in which case we store O(n) elements in the set. Space Complexity is O(n)
