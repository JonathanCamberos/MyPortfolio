...
## Solution 3: Sort Then Iterate Comparison Space Optimization - Hashmap/Representation
```python
    def containsDuplicate(self, nums: List[int]) -> bool:
...
|  Aspect  | Time Complexity | Space Complexity |  Time Remarks | Space Remarks |
| -------- | --------------- | ---------------- | ------------- |  ------------ |
| Sorting | O(n log n) | O(1) | Sorting With Complexity O(n log n) | In place sorting, no extra memory O(1) |
| Iteration | O(n) | O(n) | Iteration over list in linear O(n) + in, add operation O(1) | Set size relative to input O(n) |
| Overall | O(n) | O(n) | Iteration over list dominates, O(n) | Allocation relative to input dominates, O(n) | 
...
## Solution 3: Unicode Extension Hashmap Double Pass - Hashmap/Representation
```python
    def isAnagram(self, s: str, t: str) -> bool:
        
        # Note:
        # Normalization is required in real life:
        # - Text copied from different sources
        # - User input from different keyboards
        # - Databases with mixed normalization
...
## Solution 4: Array of 26 Single Pass - Hashmap/Representation
```python
    def isAnagram(self, s: str, t: str) -> bool:

        # Note:
        # Unicode cannot be extended for array solution 
        # since number of characters is not bounded to 26
