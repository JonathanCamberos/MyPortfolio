...
We can represent logic or conditions.
...
| Inner Loop | O(n<sup>2</sup>) | O(1) | For each element in the outer loop, the inner loop iterates through all subsequent elements with Big(O) is O(n<sup>2</sup>). Derivation [here](/Notes/sorting-searching-algorithms#double-for-loop) | No additional memory is allocated O(1) |
...
| Overall | O(n<sup>2</sup>) | O(1) | The nested loops result in O(n<sup>2</sup>) time complexity. Derivation of average case is found O(n<sup>2</sup>) is found [here](/Notes/sorting-searching-algorithms#double-for-loop). | No additional space was allocated, so it is independent of input size O(1) | 
