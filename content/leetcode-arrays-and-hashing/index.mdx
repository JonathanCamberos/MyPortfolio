...
### Brute Force (Insertion sort frequency tuples descending)
...
```python
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        # Finds the k most frequent elements using a divide-and-conquer selection approach.
        def partition(left, right, pivotIndex):
            pivot_frequency = frequency[unique[pivotIndex]]
            ...
            for i in range(left, right):
                if frequency[unique[i]] > pivot_frequency:  # Higher frequency -> "greater"
        ...
        # This avoids sorting the entire array and directly focuses on isolating the 
        ...
        def quickselectHelper(left, right, kSmallest):
            ...
            # Base Case: found the top-k partition
            if kSmallest == pivotIndex:
                return 
            
            # Recursively select the pivot
