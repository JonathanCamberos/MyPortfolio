...
```python
    def containsDuplicate_bruteforce(self, nums: List[int]) -> bool:
        ...
        # overall: time complexity O(n^2) / space complexity O(1)
        return False
```
...
```python
    def isAnagram(self, s: str, t: str) -> bool:
        ...
        def mergeSort(string):
            ...
            # mergeSort 2 subarrays -> mergeSort time/space complexity
...
|  Solution  | Time Complexity | Space Complexity | Time Remark | Space Remark |  
| ---------- | --------------- | ---------------- | ----------- | ------------ |
| Brute Force | O(n<sup>2</sup> * k) | O(1) | For each pair of strings, we compare them leading to O(n<sup>2</sup>). Using a Hashmap to check if they are anagrams takes O(k). Leading to O(n<sup>2</sup> * k)  | Stores temporary intermediate groups in a list |
...
```python 
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        # time complexity: O(n log n) to split + merge array 
        # space complexity: overall O(n) = temporary array during merging O(n) + stack memory O(log n)
        def mergeSort(word):
        
            # base case of list with 1 element
            # time complexity: O(1)
            # space complexity: O(1)
            if len(word) <= 1:
                return word
            ...
            # Explicitly handle any remaining elements
            # time complexity: worst case of O(n)
            if i < len(left):   # If elements remain in `left`
                sorted.extend(left[i:])
            else:               # If elements remain in `right`
                sorted.extend(right[j:])
        ...
        # for each word, sort + add to corresponding hashmap
        for word in strs:
