...
Thus, Hashmaps are only efficient as the chosen function.
...
In this case there are only 5 buckets or arrays keys would be hashed to.
After inserting n elements, performing a lookup would for some element k, 
...
|  Solution  | Time Complexity | Space Complexity | Time Remark | Space Remark |  
| ---------- | --------------- | ---------------- | ----------- | ------------ |
| Brute Force (merge sorting both strings) | O(n log n) | O(n) | Sorting an individual string O(n log n) dominates as comparing strings by character sequentially is linear O(n) | No additional memory is allocated O(1) |
...
```python
    def isAnagram(self, s: str, t: str) -> bool:
        ...
        def merge(left, right):
            ...
            # If elements remain in `left`
            sorted.extend(left[i:])
            # If elements remain in `right`
...
|  Solution  | Time Complexity | Space Complexity | Time Remark | Space Remark |  
| ---------- | --------------- | ---------------- | ----------- | ------------ |
| Brute Force (merge sorting + hashmap key value list) | O(n<sup>2</sup> * k) | O(1) | For each pair of strings, we compare them leading to O(n<sup>2</sup>). Using a Hashmap to check if they are anagrams takes O(k). Leading to O(n<sup>2</sup> * k)  | Stores temporary intermediate groups in a list |
...
|  Solution  | Time Complexity | Space Complexity | Time Remark | Space Remark |  
| ---------- | --------------- | ---------------- | ----------- | ------------ |
| Brute Force (Sort all frequencies descending) | O(n + m log m) | O(n + k) | Sorting m unique elements domi | |
| Stack PushPop | O(n + m log m) | O(n + k) |  |  |
| Frequency Buckets | O(n) | O(n) | | |
| Heap | O(n + m log k) | O(n + k) | | | |

### Brute Force (Sort all frequencies descending)

```python
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        
        # space complexity: count for all ints O(n)
        freq = defaultdict(int)

        # time complexity: iterate over all nums O(n)
        for num in nums: 
            freq[num] += 1

        # generate tuple of [(int, count), ...], ascending by count
        sorted = sorted(freq.items(), key=lambda tuple: tuple[1])
        sorted = sorted[::-1] # reverse list
        ...
        for i in range(k):
            result.append(sorted[i][0]) # grabbing int key from tuple [(int, count), ...]
