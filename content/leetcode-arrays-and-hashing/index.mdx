...
Leetcode problems with elegent solutions using hashing. 
...
[(double table size * load factor) - current n elements] = additional space
...
### Application
Hashmaps, with quick O(1) average time for insert(), lookup(), and delete(), 
...
### Constraints
Array Length: 1 &le; nums.length &le; 10^5 

Integer Size: -10^9 &le; nums[i] &le; 10^9  
...
| ---------- | --------------- | ---------------- | ----------- | ------------ |
| Brute Force | O(n<sup>2</sup>) | O(1) | For each pair of elements, we compare them, leading to quardratic number of comparisons | No additional space is used as we do not store any extra data |
| Hashmap | O(n) | O(n) | Each element is processed once, with O(1) lookup and insertion into the map, resulting in linear time | We store each each element in a hashmap, which requires additional memory proportional to the number of elements |
| Hashset | O(n) | O(n) | Same as logic as Hashmap | Same as logic as Hashmap |
...
| -------- | --------------- | ---------------- | ------------- |  ------------ |
| Outer Loop | O(n) | O(1) | Iterates over the array once for each elemenet O(n) | Uses the loop counter, no additional storage O(1) |
| Inner Loop | O(n) | O(1) | For each element in the outer loop, iterates through all subsequent elements O(n) | Uses the loop counter, no additional storage O(1). |
...
|  Aspect  | Time Complexity | Space Complexity |  Time Remarks | Space Remarks |
| -------- | --------------- | ---------------- | ------------- |  ------------ |
| Insertion | O(n) | O(n) | Iteration over the list takes linear time O(n) with insertion into Hashmap in O(1) time. Iteration dominates O(n). | Hashmap grows proportional to the number of unique elements. Worst case O(n) |
...
| Overall | O(n) | O(n) | Iteration dominates over insertion for time complexity linearly for O(n) | Memory allocation for Hashmap dominates space complexity. | 
...
|  Aspect  | Time Complexity | Space Complexity |  Time Remarks | Space Remarks |
| -------- | --------------- | ---------------- | ------------- |  ------------ |
| Insertion | O(n) | O(n) | Iteration over the list takes linear time O(n) with insertion into Hashset in O(1) time. Iteration dominates O(n). | Hashset grows proportional to the number of unique elements. Worst case O(n) |
| Lookup | O(1) | O(1) | Each loopup is O(1), performed once per element | Lookup operations do not require additional memory allocation. |
| Overall | O(n) | O(n) | Iteration dominates over insertion for time complexity linearly for O(n) | Memory allocation for Hashmap dominates space complexity. | 
...
|  Solution  | Time Complexity | Space Complexity | Time Remark | Space Remark |  
| ---------- | --------------- | ---------------- | ----------- | ------------ |
| Brute Force | O(n log n) | O(n) | Sorting dominates the complexity since comparing strings is linear O(n) | No additional space is used as we do not store any extra data |
| Hashmap | O(n) | O(n) | Each element is processed once, with O(1) lookup and insertion into the map, resulting in linear time | We store each each element in a hashmap, which requires additional memory proportional to the number of elements |
...
|  Aspect  | Time Complexity | Space Complexity |  Time Remarks | Space Remarks |
| -------- | --------------- | ---------------- | ------------- |  ------------ |
| Sorting | O(n log n) | O(n) | Sorting the strings involves recursively dividing the input and merging them back. Merge sort explanation [here](/blogs/common-formulas#merge-sort). | Temporary arrays are used during merging, leading to O(n) space |
| Comparison | O(n) | O(1) | Comparing the sorted strings character by-element takes linear time | No additional space is required for comparison |
| Overall | O(n log n) | O(n) | Sorting dominates the time complexity, comparison adds negligible overhead. | Space required for sorting is portportional to input size. |
...
|  Aspect  | Time Complexity | Space Complexity |  Time Remarks | Space Remarks |
| -------- | --------------- | ---------------- | ------------- |  ------------ |
| Building Map | O(n) | O(n) | Iterating over the first string to populate a Hashset takes linear time O(n) | Hashmap size depends on the number of unique characters in the input. Worst case O(n) |
| Editing Map | O(n) | O(1) | Iterating over the second string to edit the Hashset takes linear time O(n) | Space is reused from the Hashmap, no additional memory is allocated |
| Verification | O(n) | O(1) | Iterating over the Hashmap takes linear time O(n) | Space is reused from the Hashmap, no additional memory is allocated |
| Overall | O(n) | O(n) | Character counting and verification result in O(3n) which Big(O) abstracts to O(n). | Space usage is directly proportional to the unique character count. |
...
|  Aspect  | Time Complexity | Space Complexity |  Time Remarks | Space Remarks |
| -------- | --------------- | ---------------- | ------------- |  ------------ |
| Building Array | O(n) | O(1) | Iterating over the first string to populate the Array takes linear time O(n) | Fixed-sized array of 26 elements is used, independing of input size |
| Editing Array | O(n) | O(1) | Iterating over the second string to edit the Array takes linear time O(n) | Space is reused from the Array, no additional memory is allocated |
| Verification | O(n) | O(n) | Iterating over the Array takes linear time O(n) | Space is resued from the Array, no additional memory is allocated |
| Overall | O(n) | O(1) |  counting and verification result in O(3n) which Big(O) abstracts to O(n) | Space usage optimal due to a constant size array |
...
|  Aspect  | Time Complexity | Space Complexity |  Time Remarks | Space Remarks |
| -------- | --------------- | ---------------- | ------------- |  ------------ |
| Outer Loop | O(n) | O(1) | Iterates over the array once for each element O(n) | Uses the loop counter, no additional storage O(1) |
| Inner Loop | O(n) | O(1) | For each element in the outer loop, iterates through all subsequent elements O(n) | Uses the loop counter, no additional storage O(1). |
...
| Overall | O(n) | O(n) | Iterating dominates time complexity linearly O(n). | Building Hashmap dominates space complexity proportional to number of unique elements in hashmap O(n). |
...
|  Solution  | Time Complexity | Space Complexity | Time Remark | Space Remark |  
| ---------- | --------------- | ---------------- | ----------- | ------------ |
| Brute Force | O(n<sup>2</sup> k) | O(n) | For each string, compares with every other string for equality, resulting in quadratic pairwise checks O(n<sup>2</sup>). Counting characters for each string takes O(k).  | Stores temporary intermediate groups in a list |
| Sort Grouping - Merge sort | O(n k log k) | O(n k) | Sorting each string takes O(k log k). Iterating over all strings takes linear O(n). | Sorted strings are stored with associated groups in memory. |
| Hashmap | O(n k) | O(n k) | Iterating over all strings is linear O(n) and counting character is O(k) | Stores a Hashmap with character counts O(k) for each string O(n) |
...
```python 
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        def merge_sort(word):
            ...
            while i < len(left) and j < len(right):
                if left[i] < right[j]:
                    sorted_word.append(left[i])
                    i += 1
                else:
                    sorted_word.append(right[j])
                    j += 1
            
            sorted_word.extend(left[i:])
            sorted_word.extend(right[j:])
    ...
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        for word in strs:
            ...
            for char in word:
                ...
                key = tuple(char_count)  # Use tuple of counts as key
            
            if key not in hashmap:
                hashmap[key] = []  # Initialize a list for this key if it doesn't exist
            hashmap[key].append(word)  # Add the current word to the corresponding key group
