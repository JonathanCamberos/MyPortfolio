...
### Solution 2: char count array/tuple + hashmap
```python
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        
        # hashmap (char_count_tuple, string)
        ...
        for word in strs:
            ...
            # matches tuples to group strings 
            dic[tuple(lis)].append(word)
...
```python
    def topKFrequent(self, nums: List[int], k:int) -> List[int]:
        ...
        freq = [[] for i in range(num_buckets)]
        ...
        # decreasing for loop, grab k highest occuring 
...
This one is the first time we use an algorithm, albiet a simple one. 
