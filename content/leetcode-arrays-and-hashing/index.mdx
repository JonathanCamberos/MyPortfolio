...
|  Aspect  | Time Complexity | Space Complexity |  Time Remarks | Space Remarks |
| -------- | --------------- | ---------------- | ------------- |  ------------ |
| Frequencies | O(n) | O(n) | Iterate over list O(n) | Allocation for n elements O(n) |
| MinHeap | Best O(log k), Worst O(log n) | Best O(k), Worst O(n) | Insert BubbleUp on heap of size k or n O(log k) or O(log n) | Allocation for heap of k or n length O(k) or O(n) |
| Overall | Best O(n log k), O(n log n) | Best O(k), Worst O(n) | Iterating over list for frequency count dominates * Insert BubbleUp per element in freq count list, dominates, O(n log k) or O(n log n) | Allocation for heap of k or n length dominates, O(k) or O(n) |


## Solution 2: Descending QuickSelect BinarySearch - Hashmap/Algorithm
```python
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        def partitionSection(left, right, randPivotElemIndex):
            ...
            # Partition elements with (pivotElemFreq < freq) to left side
            # tc: iterate over subarray, n in worst case O(n)
            for i in range(left, right):

                # Higher freq element than pivot element, move to left -> 'greater'
                if pivotElemFreq < frequency[unique[i]]:  

                    # move larger freq element to left, sorting high to low 
