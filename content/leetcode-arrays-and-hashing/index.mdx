...
```python
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        # space complexity: frequency count for all ints O(n)
        count = defaultdict(int)
        ...
        # time complexity: Best case O(n), Average 
        for i in range(1, len(countTuple)):
            curr = countTup[i]
            j = i - 1

            while j >= 0 and countTuple[j][1] < curr[1]:
                countTuple[j + 1] = countTuple[j]
                j -= 1
            
            countTuple[j + 1] = curr # insert element in correct place

        # time complexity: grab k top occurring ints from frequency list O(k)        
        result = []
        for i in range(k):
            result.append(countTuple[i][0]) # grabbing int key from tuple [(int, count), ...]
