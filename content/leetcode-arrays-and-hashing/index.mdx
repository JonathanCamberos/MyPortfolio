...
### Hashmaps Use Case: Representations

We can represent an object based on a criteria.

Ex: Given a string, count the frequency of each character:
```python
    s = "aabbcc"
    freq = {}
    for char in s:
...
### Hashmap Use Case: Grouping

We can grouping elements based on a criteria.

Ex: Group strings that are anagrams of each other:
```python
...
### Hashmap Use Case: Dynamic Programming Storing Subproblems

We can store solutions to subproblems to avoid redundant calculations.

Ex: Fibonacci number solutions with memoization:
```python
    def fib(n, memo={}):
        if n <= 1:
            return n
...
### Hashmap Use Case: Backtracking Storing Potential Solutions for Pruning

We can cache paths or partial solutions to prune the search space effectively
...
### Hashmap Use Case: Representing Relationships 

We can represent relationships between items.
...
### Find the Bug: Set -> String Key (Set Frequency Count)
```python
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
...
### Solution 1: Hashmap Array of 26 -> Tuple Key
```python
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
...
### Solution 2: Hashmap Array of 26 -> String Key
...
### Solution 1: Hashmap -> Bucket Sort
...
### Solution 3: Quickselect (high to low)
```python
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
...
### Solution 4: Quickselect (low to high)
```python
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
...
### Solution 1.1: Defaultdict Matrix
```python 
    def isValidSudoku(self, board: List[List[str]]) -> bool:
