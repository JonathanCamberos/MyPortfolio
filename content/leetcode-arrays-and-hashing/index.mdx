...
### What is a Hashmap
A Hashmap is a specific implementation of a dictionary. 
A dictionary is just a direct mapping of keys->values:

dictionary: ['a': 2, 'b': 2...] 

Thus, a Hashmap is just a dictionary that maps keys->values using a hash function.

Hashmaps aka Hashtables, is a common use case for hashing.
And in this use case, we choose hashing to maxmize randomness, and minimize collisions.
...
So we learn, Hashmaps are only efficient as the choosen function.
...
Thus, when certain fraction of the table size (ex: 0.75% full), the table must rehash and create a bigger table/reinsert elements 

The efficiency of a hash table decreases significally without rehashing, as this leads to increased collisions
and increased time to resolving collisions (through chaining or probing).
...
Rehashing is thus, expensive as we get O(n) for resizing and reinserting n elements.

Usually, when the load factor is reached, the table size doubles to 2n.
And usually, a common load factor is 0.75 or 75% full.

If the table is resized to 2n, it can accomodate (2n * 0.75) - n more elements:

(double table size * load factor) - elements currently present
...
Due to their quick O(1) average time for insert(), lookup(), and delete(), 
they are great for storing key pairs of lists of elements to perform more complex tasks. 

This is commonly the first step of adopting a 'leetcode' mindset
where both time complexity and space complextiy are important for performance.
...
### Constraints
length: 1 &le; nums.length &le; 10^5 

integer: -10^9 &le; nums[i] &le; 10^9  
...
Lower bound: 
We need to traverse the list at least once to peak at every element in it. 
So the best possible time complexity is O(n).

Since we need to track every elemeent, best space complexity is O(unique(n))
...
[Hashmap Applications](/blogs/common-formulas#burger)
...
No additional data strcutures are used. Only a few variables are maintained.

Space Complexity is O(1).

### Solution 1: hashmap/defaultdict
```python
...
Time Complexity:  O(n) -> loop and dictionary operations
