...
The algorithm efficiently constructs that path by traversing edges while removing them,
ensuring every edge is visited exactly once.

## Graph Requirements
1. Directed or Undirected
2. Must satisfy Eulerian conditions:
    - Eulerian Circuit:
        Directed: Every vertex has equal in and out degrees 
        Undirected: Every vertex has even degrees

    - Eulerian Path:
        Exactly 0 or 2 vertices have unequal degree
...
## Space Complexity
Stack: Up to O(V) in worst case
Path Storage: O(E + 1) vertices

O(V + E)

## IRL Use Case
- Route Planning
    Postman problems (traverse each street exactly once)

- Puzzle Solving
    Draw shapes without lifting a pen
