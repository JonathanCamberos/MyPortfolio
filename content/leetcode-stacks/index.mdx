...
```python
    def largestRectangleArea(self, heights: List[int]) -> int:
        ...
        #     which means the new height candidate is taller than at least 1 older bar.
        #     As we generate the area, the width of the rectangle is determined by 
        #     the distance between the bar which was not popped and the new height candidate
        #     The new height candidate becomes the new top of the stack, and is compared against new walls,
        #     which may or may not be taller or shorter.
