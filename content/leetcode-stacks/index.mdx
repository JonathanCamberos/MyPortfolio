...
```python
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        # so then we can use the implication that farther cars with lower arrival times than the car in front off them will catch up.
        # Once they catch up, that fleet will be determined by the car they just caught up with.

        # As we iterate from farthest to closest: 
        # - any car that catches up to a car ahead of them forms a fleet, 
        #   with the head of the fleet being the car that was just caught up to 
        #
        # - any car that is not caught up with forms a new fleet,
        #   with the head of the fleet being that car that was never caught up to.

        # Cars ordered from farthest to closest to target, with bars representing time to target:
        #                  
        #               
        #         *        *            (estimated)
        #      *  *     *  *            car 0 has a lower time than car 1, it catches up    (fleet)       1
        #      *  *  *  *  *  *         car 2 has a lower time than car 1, it gets away     (new fleet)   2
        #     ------------------        car 2 has a lower time than car 3, it catches up    (fleet)       2
        #      0  1  2  3  4  5         car 3 has a lower time than car 4, it catches up    (fleet)       2
        #    farther --> closer         car 5 has a lower time than car 4, it gets away     (new fleet)   3

        # Implication:
        # If a farther car has a lower arrival time (less stars), 
        # then that car will catch up with the car ahead of it
        ...
        for (pos, spd) in sorted(zip(position, speed), reverse=True): 
            ...
            # Check: if stack is empty, create first fleet
