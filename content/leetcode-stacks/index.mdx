...
```python
    def largestRectangleArea(self, heights: List[int]) -> int:
        ...
        #      *  *  *  *     +    *       ==>    *  *  *  *  *   ==>  *  A  A  A  A  
        #     ------------   new  ---  pop off   ---------------      ---------------
        #    older --> newer        left candidates  

        # Popping Rule:
        # We keep popping as long as the stack as taller bars than the current smaller bar.
        ...
        #     is the smallest height encountered (covered by all other bars), 
        #     so it spans from 0 -> i, which is a width of i
        #
        #  2. The stack does not get completely popped, which means the small height candidate
        #     becomes the new left hand side of the rectangle to be generated (replaces the lower bound of 0 represented by the sentinel) 
        ...
        for i in range(len(heights)):
            ...
            while stack and heights[stack[-1]] > heights[i]:
                ...
                # that was smaller than all of the previous heights
                # That means, that the small bar was covered by all of the taller bars it popped off,
                # so we can use that bar as the width and use its height.
                # so we are guaranteed width of 0 through i, width length of i
                if not stack:
                    width = i

                # Check: Stack is not empty
                # Current tallWall is taller than some other left wall to the left of it,
                # which means that it is bounded by that shorter left wall,
                # which means we cannot use it in the width calculation and need to account for it by
                # doing a -1 to account for the left wall that we are not including
                else:
                    leftWallIndex = stack[-1]
                    width = i - leftWallIndex - 1
