...
```python
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        # We calculate the the time required for each car to reach the target.
        # We sort the cars by starting position descending (farthest from target first)
        # so we can process cars in the order of "approaching the target".

        # Note:
        # Monotonic stack: maintains a monotonic decreasing stack of times to target (top = closest to end)
        ...
        # space complexity: stack tracks arrival time
        stack = []
        ...
        for (pos, spd) in cars.items():
            ...
            # If current car's time is <= stack top, it merges with that fleet
            # If current car's time is > stack top, it forms a new fleet.
            # Check: stack is not empty
            # Invariant: stack[-1] represents the arrival time of the right most 
            # (nearest to target) car fleet we have seen so far as we iterate from right to left
            # right being car farthest from end goal and left being car closest to end goal
