...
```python
    def dailyTemperatures(self, temperatures: List[int]) -> List[int]:
        ...
        # Dynamic Programming:
        # Uses a jump based approach via leveraging results from previously computed indices.
        # Avoids a  stack by simulating the search for the next warmer temperature
        # by iterating backwards and keeping track of the nearest hottest day,
        # effectively building a list of shortcut paths towards the next warmer day

        # Jump Traversal:
        # If we are trying to find the hottest day for index i, we know that our first candidate is the
        # following day. 
        # If the following day is not the nearest hottest day, then we know that the first candidate and
        # the following day share the next following day.
        
        # So essentially, once we know the next warmer day for some index j, we can skip
        # forward using the result array we are building to find the next warmer day.
        ...
        for i in range(n - 2, -1, -1):
            ...
            # Check: if temperature hotter than current hottest day
            # Implies: No need for wait days calculation, as no future hotter day exists 
            if temperatures[i] >= temperatures[hottest_day]:
                # update hottest day
                hottest_day = i

            # Check: if temperature is colder than current hottest day
            # Then: need to calculate wait time, find the nearest hot day
            else:

                # First candidate is the following day
                tempCheckIndex = i + 1

                # Check: if candidate is hotter than current day
                while temperatures[tempCheckIndex] <= temperatures[i]:
                    
                    # Implies: candidate is not a hotter day
                    # Implies: need a day hotter than the current candidate
                    # Then: we can check candidates own hot day as the next candidate
                    tempCheckIndex += dp[tempCheckIndex]

                # Check: hotter day found
                # Then: set day index in building list
                dp[i] = tempCheck - i 
