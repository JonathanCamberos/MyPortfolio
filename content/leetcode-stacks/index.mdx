...
### Stack Use Case: Tracking Nested or Hierarchical Structures
...
### Stack Use Case: Backtracking by Tracking History or State
...
### Stack Use Case: Monotonic Property Maintenance
...
### Stack Use Case: Simulating Recursion or Call Stacks
...
### Stack Use Case: Expression Evaluation and Parsing
...
### Stack Use Case: Backtracking
We can cache paths or partial solutions to prune the search space effectively.

Ex: Generate all valid combinations of n pairs of parentheses
```python 
    def generateParentheses(n: int) -> list[str]:
        ...
        def backtrack(current: str, open_count: int, close_count: int):
            ...
            # (Inherently prune invalid cases where open_count >= n)            
            if open_count < n:
                backtrack(current + "(", open_count + 1, close_count)
            
            # Add a closing parenthesis if it's valid 
            # (Inherently prune invalid cases where close_count >= open_count)            
...
### Stack Use Case: Partitioning
We can use a stack to partition or segment a sequence into meaningful parts 
by tracking boundaries or "cut points" as we iterate through the input.
