...
```python
    def dailyTemperatures(self, temperatures: List[int]) -> List[int]:
        ...
        for i in range(n-1, -1, -1):
            ...
            # Invariant: current cold day could serve as potential more recent hot day for earlier days, append to stack
...
```python
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        # We sort the cars by closest to target to farthest from target 
        # (higher value is closer, smaller value is further).
        # We can use the implication that farther cars with lower arrival times 
        # than the car in front off them will catch up and form a fleet.
        # Once they catch up, that fleet will be determined by the slower car they just caught up with.

        # As we iterate from farthest to closest: 
        #  start with a car
        #  if that car has a lower (quicker) arrival time than the car in front of it, it will join the car ahead (join fleet)
        #  if the car has a higher (slower) arrival time than the car in front of it, it will stay behind the car ahead (form new fleet)

        # Cars ordered from farthest to closest to target, with bars representing time to target:
        #                  
        #                                     (estimated)
        #   Time to                           car 0 forms the first fleet                                               1
        #   target   1  4  3  7  1  2         car 1 has lower time than car 0, does not catch up, forms second fleet    2
        ...
        # Simple Implication:
        # If a farther car has a lower arrival time (less stars) than the fleet ahead of it, 
        # then that car will catch up and join the fleet

        # Monotonic stack: 
        # Maintains a monotonic increasing stack of times to target
        # which represent the times of the fleets
        
        # sc: stores times for up to n fleets O(n)
        stack = []

        # Pair each cars position with its speed
        cars = list(zip(position, speed))

        # Sort cars by position descending, closest to the target first has higher number
        # tc: timSort cars by starting position distance descending O(n log n)
        cars.sort(reverse=True)

        # Iterate through closer to farthest cars
        ...
        for (pos, spd) in cars:
            ...
            # Farthest fleet:
            # stack[-1] will hold the lowest time to target, for the fleet closest to the target

            # No fleet will form:
            # If current car is faster (time to target <= stack top), it will catch up with fleet and merge
            # If farther car has longer arrival time, it will never catch up to next car

            # Check: if stack is empty, car automatically becomes fleet -> push()
            # Check: if stack is not empty, check if car is slower than current fleet (does not catch up) -> car becomes new fleet -> push()
            if not stack or currTimeToTarget > stack[-1]:

                # Update new fleet at top of stack
                stack.append(currTimeToTarget)

            # If current car is faster (time to target <= stack top), it will catch up with fleet and merge
            # If farther car has shorter arrival time, it will catch up to next car,
            # and the new fleet is then determined by slower car which was caught up to.
            else:
                continue
            # Check: car is faster than current fleet and catches up -> do nothing (fleet stays the same)
        
        # number of fleets represented by number of fleets on stack
