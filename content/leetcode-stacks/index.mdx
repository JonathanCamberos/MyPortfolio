...
```python
    def generateParenthesis(self, n: int) -> List[str]:
        ...
        dp[0] = [""]  # Base case: one valid combination for n = 0

        for i in range(1, n + 1):  # Compute DP for 1 to n
            for j in range(i):  # Iterate through partitions
                # Append combinations of dp[j] and dp[i - 1 - j]
                for left in dp[j]:
