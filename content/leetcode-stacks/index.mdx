...
Topics:  Array, Stack, Monotonic Stack, Two Pointer, Dynamic Programming
...
```python
    def dailyTemperatures(self, temperatures: List[int]) -> List[int]:
        ...
        for i in range(n-1, -1, -1):
            ...
            # Check: stack is non empty
            # Check: if current temperature[i] breaks monotonic decreasing, 
            # we have found a cold temperature
            while stack and temperatures[stack[-1]] <= temperatures[i]:
                stack.pop()
...
### Solution 1: Forward Iteration Monotonic Increasing of Higher Bar Height Index - Stack/Monotonic Property Maintenance
```python
    def largestRectangleArea(self, heights: List[int]) -> int:
        ...
        # If the monotonic increasing height is valid again, it means nothing covers the current
        # height candidate so we add it to the stack
        ...
        for i in range(len(heights)):
            ...
            while stack and heights[stack[-1]] > height[i]:
                ...
                # Before pop
                # h: height candidate
                # stack[-1]: tallest height (guaranteed covers height candidate)
            ...
            # monotonic increasing is maintained, curr height is no longer 
            # covered by the tall bars, add to stack 
            stack.append(i)
...
### Solution 2: Reverse Iteration Monotonic Increasing of Higher Bar Height Index - Stack/Monotonic Property Maintenance 
```python
    def largestRectangleArea(self, heights: List[int]) -> int:
        ...
        for i in range(n-1, -2, -1):
            ...
            # append a sentinel to flush stack at the end 
            # height of left wall
            curr_height = -1 if i == -1 else heights[i]
            ...
            while stack and heights[stack[-1]] > curr_height:
                ...
                # If stack is empty, then current height is the smallest encountered so far
                # and has popped all taller walls
                # this implies all taller walls cover its height
                # so current height goes from 
                if not stack:
                    width = (n - 1) - i
                else:
                    width = (stack[-1] - 1) - i
