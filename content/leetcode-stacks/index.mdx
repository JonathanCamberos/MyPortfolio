...
```python
    def evalRPN(self, tokens: List[str]) -> int:
        ...
        for token in tokens:
            ...
            # found number, cast to int and push to stack
            if token not in "+-*/":
                stack.append(int(token))
            
            # found operation, grab top 2 numbers from stack and compute
            # numbers are in reverse order, so pop b first, then a.
            else:

                # tc: pop two operands from stack O(1)
        ...
        # top of stack holds with final number
...
```python
    def generateParenthesis(self, n: int) -> List[str]:
        ...
        def helper(current: List[str], num_open: int, num_closed: int):
            ...
            # Base case: 
            # If the length of the current combination is 2 * n, we found a valid combination
            if len(current) == n * 2:
                # tc: concat add to res for 2n length O(n)
                res.append(current)
                return

            # Recursive case 1: add '('
            # Check: open count is less than total expected
            # Implies: string is still valid
            # Ensures: '(' will create a new valid string
            if num_open < n: 
            ...
            # Check: closed count is less than open count
            # Implied: string is valid from previous, has a '(' that is waiting for a closing
            # Ensures: The ')' will have a matching open
            if num_closed < num_open:
                # tc: concat per iteration O(n)
                helper(current + ")", num_open, num_closed + 1)

        # Start backtracking with empty list and 0 open and 0 closed brackets
        helper("", 0, 0)
    ...
    def generateParenthesis(self, n: int) -> List[str]:
        ...
        def backtrack(current, openCount, closeCount):
            ...
            if openCount < n: 
                ...
                # Recursive:
                # Explores all combinations from new state
            ...
            if closeCount < openCount:
                ...
                # Recursive:
                # Explores all combinations starting from new state
...
```python
    def generateParenthesis(self, n: int) -> List[str]:
        ...
        # Explicit stack:
        # Simulates backtracking process iteratively.
        # Each stack element stores the current string and counts of open and closed parentheses.
        # Allows state tracking without recursion.
...
## Solution 1: [Monotonic] Forward Iteration Monotonic Decreasing Stack of Cold Temps - Stack/Monotonic Property Maintenance
```python
    def dailyTemperatures(self, temperatures: List[int]) -> List[int]:
...
## Solution 2: [Monotonic] Reverse Iteration Monotonic Decreasing Stack of Warm Temps - Stack/Monotonic Property Maintenance
```python
    def dailyTemperatures(self, temperatures: List[int]) -> List[int]:
...
## Solution 3: [Dynamic Programming] Reverse Iteration With Jump Traversal Using Dynamic Programming Building Future Warm Temperatures List - Stack/Algorithm
```python
    def dailyTemperatures(self, temperatures: List[int]) -> List[int]:
        ...
        # This effectively builds a shortcut path toward the next warmer day
        ...
        # Tracking index of hottest day seen so far with reverse iteration right to left
        # Start with hottest day set to last element
        hottest_day = n - 1
        ...
        for i in range(n - 2, -1, -1):
            ...
            # Implies: No need for wait days calculation, as no future hotter day exists 
            if temperatures[i] >= temperatures[hottest_day]:
                # update hottest day index
                hottest_day = i

            # Implies: a hotter day exists
            # Then: use hot day history to find nearest hotter day
            else:

                # Start with following day
                tempCheckIndex = i + 1

                # while hotter day not found
                while temperatures[tempCheckIndex] <= temperatures[i]:
                    # jump to candidate's hottest day index
                    tempCheckIndex += dp[tempCheckIndex]

                # found hotter day, set in list
                dp[i] = tempCheck - i 
...
## Solution 1: [Monotonic] Dragging Right Wall Height Over The Array And Building Area By Creating Monotonic Increasing Guaranteeing Area - Stack/Monotonic Property Maintenance
```python
    def largestRectangleArea(self, heights: List[int]) -> int:
        ...
        # Sentinel: 
        # We need a left hand side limiting wall height, so we start with 0.
        # This allows us to have a monotonic increasing rule upon grabbing the first height.
        # This also allows us to follow implication 1, as the area being generated from 0 -> i.

        # sc: stores heights for up to n heights O(n)
        stack = []
        max_area = 0

        # Sentinel: 0 to start the monotonic increasing rule
        heights.append(0)
        ...
        for i in range(len(heights)):
            ...
            while stack and heights[stack[-1]] > heights[i]:
                ...
                # The current tallest bar that was on top of the stack, was actually a short bar
                # that was smaller than all of the previous heights and we have reached the left hand side of the grid.
                # That means, that the small bar was covered by all of the previous taller bars it popped off,
                # so we can use that bar as the width and use its height and go from 0 -> i.
                if not stack:
                    width = i

                # Check: Stack is not empty
                # Current tallWall on top of the stack is taller than some other left wall to the left of it.
                # Implies: The current area generated is being bound in width by the shorter left wall on the top of the stack.
                # Implies: We cannot include that left wall in the width calculation and need to account for it by doing a -1
                else:
                    leftWallIndex = stack[-1]
                    width = i - leftWallIndex - 1
...
## Solution 2: [Monotonic] Double Dragging Reverse Iteration Monotonic Increasing of Higher Bar Height Index - Stack/Monotonic Property Maintenance 
```python
    def largestRectangleArea(self, heights: List[int]) -> int:
