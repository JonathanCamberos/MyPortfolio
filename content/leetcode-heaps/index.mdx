...
```python
    def lastStoneWeight(self, stones: list[int]) -> int:
        # Note:
        # 1. We need to repeatedly take the two heaviest stones.
        # 2. Python's heapq is a min-heap, so to simulate a max-heap:
        #    store each stone as its negative value.
        # 3. Pop the two largest (most negative) stones, smash them, and push the result
        #    back into the heap (if not zero).
        # 4. Continue until at most one stone remains.
        # 5. Return that stone's weight, or 0 if no stones left.
        #
        # Time Complexity: O(n log n) — heapifying + repeated pop/push operations.
        # Space Complexity: O(n) — storing all stones in heap.
        ...
        while len(maxHeap) > 1:
            ...
            first = -heapq.heappop(maxHeap)  # largest
            second = -heapq.heappop(maxHeap) # second largest

            if first != second:
                # Push the difference back (negative for max-heap)
                heapq.heappush(maxHeap, -(first - second))

        # Return last stone if exists, else 0
    ...
    def kClosest(self, points: list[list[int]], k: int) -> list[list[int]]:
        ...
        for x, y in points:
            ...
            if len(max_heap) < k:
                ...
                if dist < -max_heap[0][0]:  # if closer than farthest in heap
...
```python
    def kClosest(self, points: list[list[int]], k: int) -> list[list[int]]:
        # Helper: squared distance to origin
        def dist(i: int) -> int:
        ...
        def quickselect(left: int, right: int, k_smallest: int):
            """Sort the array such that the k_smallest elements are in first k_smallest slots."""
            if left >= right:
                return
            ...
            # Recurse into the part that contains the k_smallest
            if k_smallest < pivot_index:
                quickselect(left, pivot_index - 1, k_smallest)
            elif k_smallest > pivot_index:
                quickselect(pivot_index + 1, right, k_smallest)
            # else pivot_index == k_smallest → done
...
class Twitter:
    ...
    def postTweet(self, userId: int, tweetId: int) -> None:
        ...
        self.time += 1
        # Append tweet to the user’s tweet list
