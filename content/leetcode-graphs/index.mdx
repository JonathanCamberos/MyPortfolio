...
## Solution 1: Recursive DFS With Tracking Visited Set - Graph/something
```python
...
## Solution 2: Recursive DFS With Turning Visited Land Into Water Tracking - Graph/something
```python
    def numIslands(self, grid: List[List[str]]) -> int:
...
```python 
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        # Unlike BFS or DFS, we do not traverse the graph recursively or iteratively.
        # Union-Find maintains a parent pointer and rank (height) to track connected components efficiently.
        # No backtracking is involved, we never revert a union, we only merge sets.
        # Path compression ensures find() is nearly O(1) amortized.

        # Island To Water Mimicking With Union() Same Root Ignoring:
        ...
        # with water: once two parts are connected, we never need to process the connection again.
        # This avoids duplicate work, just like marking visited cells in DFS/BFS. The union by rank
        # ensures that the tree stays shallow, keeping operations efficient.
