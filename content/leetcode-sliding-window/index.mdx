...
```python
    def checkInclusion(s1: str, s2: str) -> bool:
        ...
        # Note:
        # 1. Use fixed sliding window of length len(s1)
        # 2. Maintain two frequency arrays (size 26) for O(1) updates
        # 3. Maintain `matches` count to avoid full frequency comparison
