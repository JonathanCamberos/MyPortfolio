...
```python
    def maxProfit(self, prices: List[int]) -> int:
        ...
        # Note:
        # Sliding Window
        # 1. left points to day we buy (lowest price so far candidate)
        # 2. right scans forward for a day to sell (highest day possible)
        # 3. Get profit:
        ...
        # Lowest Buy Day so far, curr day price
        left, right = 0, 1
        ...
        while right < len(prices):
            ...
            # equal or lower buy day found, update buy day
            else:
                left = right
...
```python
    def maxProfit(self, prices: List[int]) -> int:
        ...
        # Note:
        # 1  Optimization of DP table to O(1) space
        # 2  dp_i_0 = max profit on current day with no stock
        #    dp_i_1 = max profit on current day with stock
        # 3  Transition is identical, only store previous day values
