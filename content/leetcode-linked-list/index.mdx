...
```python
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        while curr:
            ...
            # grab or create and grab random deepCopy
            if curr.random:
...
```python
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        # 3. Detach copy list from original (restoring original list) + creating deepCopy list
        # 4. Return copy head
        ...
        curr = head
        while curr:
            # grab intertwined deepCopy, assign random from original
            if curr.random:
                curr.next.random = curr.random.next
            # iterate skipping interweaved nodes
            curr = curr.next.next

        # Iteration 3:
        # Split into original and copied lists
        # grab original: A -> A' -> B ... (grabbing A)
        curr = head
        # grab deepCopy: A -> A' -> B ... (grabbing A')
        deepCopyHead = head.next

        # time complexity: iterate over list of 2n length O(n)
        while curr:
            # interweaved node copy
            deepCopy = curr.next
            # disconnect original node from interweaved node
            curr.next = deepCopy.next
            # if we have not reached end of copy list
            # remaining: A -> A' -> B -> B' (point A' -> B')
            # end: B -> B' -> None (B'.next == None)
            if deepCopy.next:
...
### Solution 1: Depth First Recursion - Linked List/Simple Traversal
```python
    def addTwoNumbers(self, l1: ListNode, l2: ListNode) -> ListNode:
        ...
        def recursiveSum(n1, n2, carry):
            
            # Base Case:
            # All lists and carry are empty
            # Attach tail of new list -> None
            ...
            # attach new node via recursive call
            node.next = recursiveSum(n1.next if n1 else None,
...
```python
    def findDuplicate(self, nums: List[int]) -> int:
        ...
        # max_bits determines number of bit positions to check
        ...
        for bit in range(max_bit):
            ...
            # count expected set bits in range
            count_range = sum((i & mask) > 0 for i in range(1, n + 1))

            # if nums count exceeds expected, duplciate has this bit set
...
```python
    def findDuplicate(self, nums: List[int]) -> int:
        ...
        # Note:
        # 1. Duplicate number must satisfy pigeonhole principle
        # 2. Binary search value range [1..n]
        # 3. Count values <= mid to determine which half contains duplicates
        #   If count > mid -> duplicates in lower half
        #   Else -> duplicates in upper half
        ...
        while left < right:
            ...
            # if too many <= mid, duplicate must be in lower half
            if count > mid:
                right = mid
            else:
                left = mid + 1
...
```python
    def findDuplicate(self, nums: List[int]) -> int:
        ...
        # Note:
        # When slow and fast meet (cycle detected),
        # fast has traveled exactly 2x the distance of slow.
        # If we move slow to the beginning
        # and move fast at the same distance of slow,
        # both slow and fast will move the same distance slow did the first time
        # and meet at the start of the cycle.

        # Note:
        # 1. Treat array as linked list: index -> nums[index]
        # 2. Duplicate guarantees cycle in linked list representation
        # 3. Phase 1: Detect cycle (slow = nums[slow], fast = nums[nums[fast]])
        # 4. Phase 2: Move slow to head, advance both one step at a time to find cycle start (duplicate)
...
class Node:
    def __init__(self, key: int, val: int):
        # Note:
        # 1. key -> value node
        # 2. prev and next pointers to allow linking in both directions
        self.key = key
        self.val = val
        self.prev = None
        self.next = None
...
class LRUCache:
    def __init__(self, capacity: int):
        # Note:
        # 1. Use a hashmap for O(1) lookups: key -> Node
        # 2. Use a doubly linked list to maintain recency order
        # 3. Dummy head/tail nodes avoid edge condition checks
    ...
    # time complexity: O(1)
    # space complexity: O(1)
    def _new_MRU(self, node: Node):
        # Note:
        # Insert node right after dummy head to mark as most recently used
    ...
    def get(self, key: int) -> int:
        # Note:
        # 1. Check if key exists in O(1) using hashmap
        # 2. If exists, move it to the end (most recently used)
        # 3. Return its value
    ...
    def put(self, key: int, value: int) -> None:
        # Note:
        # 1. If key exists, remove old node first
        # 2. Insert new node at front (most recently used)
        # 3. If capacity exceeded, remove LRU node from tail and delete from hashmap
        ...
        # Move new node to front (MRU)
        self.cache[key] = node
        self._new_MRU(node)

        # if above capacity, remove from end of list (least recently used)
...
### Solution 2: Automatic Doubly Linked List With Hashmap via Python Ordered Dict  - Linked List/Simple Traversal
```python
...
class LRUCache:
    def __init__(self, capacity: int):
        # Note:
        # OrderedDict in Python is implemented
        # internally as a doubly linked list + hashmap (same as our solution):
        # 1. Use a hashmap for O(1) lookups: key -> Node
        # 2. Use a doubly linked list to maintain recency order
        # 3. Dummy head/tail nodes avoid edge condition checks
    ...
    def put(self, key: int, value: int) -> None:
        ...
        # 2. If key does not exist, insert it and mark as recently used
        # 3. If capacity exceeded, remove least recently used (first in linked list)
        if key in self.cache:
            self.cache.move_to_end(key) # mark as recently used
        self.cache[key] = value
        if len(self.cache) > self.capacity:
            self.cache.popitem(last=False)  # Remove LRU (front of linked list)
...
```python
    ...
    def mergeKLists(self, lists: List[Optional[ListNode]]) -> Optional[ListNode]:
        ...
        # Note:
        # 1. Use a min heap to always pull the smallest node across all lists
        # 2. Heap stores (value, list_index, node) to avoid ListNode comparison errors
        # 3. Always push the next node from the same list when one node is popped
        # 4. Tail pointer builds the merged sorted list in O(1) append steps
        ...
        for i, node in enumerate(lists):
            if node:
                # i is used to break ties
                heappush(min_heap, (node.val, i, node))  

        dummy = tail = ListNode(0)

        # Pop smallest node, attach to merged list, push from same list
        while min_heap:
...
```python
    def mergeKLists(self, lists: List[Optional[ListNode]]) -> Optional[ListNode]:
        ...
        # Note:
        # 1. Repeatedly merge lists in pairs to reduce total merges
        # 2. Merge two sorted lists in O(n) time with O(1) extra space
        # 3. Pair merging reduces number of lists by ~half each iteration
        # 4. Overall complexity is O(N log k) from log k merge rounds
        ...
        # time complexity: for each mergeTwo O(n)
        def mergeTwo(l1, l2):
            # Note:
            # 1. Standard two pointer merge of two sorted linked lists
            # 2. No extra arrays - relink existing nodes
            dummy = tail = ListNode(0)
            ...
            # fancy way to merge non empty list
            tail.next = l1 or l2
...
```python
    def mergeKLists(self, lists: List[Optional[ListNode]]) -> Optional[ListNode]:
        ...
        # Note:
        # 1. Collect all values from all lists into a single Python array
        # 2. Sort the array in O(N log N)
        # 3. Build a new linked list from sorted values
        # 4. Simpler implementation but higher complexity than 
        ...
        # sort values: python default timSort()
        nodes.sort()
...
```python
    def reverseKGroup(self, head: Optional[ListNode], k: int) -> Optional[ListNode]:
        ...
        # Note:
        # 1. Check if there are at least k nodes ahead (otherwise leave remainder as is)
        # 2. Reverse the first k nodes
        # 3. Recurse on the remaining nodes
        # 4. Connect reversed head with recursively reversed remainder
...
```python
    def reverseKGroup(self, head: Optional[ListNode], k: int) -> Optional[ListNode]:
        ...
        # Note:
        # 1. Use dummy node to handle head swaps cleanly
        # 2. For each group:
        #   Find the kth node
        #   Reverse the group in place
        #   Connect previous group's tail to head of reversed group
        # 3. Continue until remaining nodes < k
        
        # Reverse a linked list segment [start, end)
        # time complexity: O(k)
