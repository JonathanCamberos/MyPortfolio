...
```python
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        # Dictionary to store original node -> copied node
        memo = {}
        
        # Step 1: Traverse original list
        while head:
            
            # Step 1A: Copy current node if not already created
            if head not in memo:
                memo[head] = Node(head.val)
            copy_node = memo[head]
            
            # Step 1B: Copy random pointer if exists
            if head.random:
                if head.random not in memo:
                    memo[head.random] = Node(head.random.val)
                copy_node.random = memo[head.random]
            
            # Step 1C: Link copied node to result list
            copy_tail.next = copy_node
            copy_tail = copy_tail.next
            
            # Step 1D: Move forward in original list
            head = head.next
        
        # Return deep-copied head (dummy.next skips placeholder node)
...
### Solution 3: In Place Interleaving Technique O(1) - Linked List/Simple Traversal
```python
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        # Step 3: split into original and copied lists
        curr = head
        copy_head = head.next
        while curr:
            copy = curr.next
