...
```python
    def reverseList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        # Base case: 
        # List is empty or has only one node -> found new head
        if not head or not head.next:
            return head

        # Receiving base case: new head
        ...
        # Detach current head from linked list
        head.next = None

        # Continue passing back new head from the deepest recursive call
