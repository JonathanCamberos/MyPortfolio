...
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # Odd List Len: slow -> middle element
        # left is 1 element longer

        # Even List Len: mid -> 2nd middle element
        ...
        # Odd List Len: start of right list
        # setting curr to slow.next, which is saying mid.next
        # left will be 1 element longer (keeps mid element),


        # Even List Len: second element of right list
        # setting curr to slow.next, which is saying mid2.next
        # left is 2 elements longer (keeps 1st and 2nd mid elements)

        curr = slow.next

        # Explicit Disconnect:
        # Set tail for left list: slow -> None
        slow.next = None 
        ...
        # Implicit Disconnect Trick:
        # Even/Odd Len Case:
        # Regardless of difference in length between right and left list,
        # since right.next = next_left,
        # this automatically connects the 1 or 2 extra nodes once 
        # while right terminates (since right list is shorter)
        # since we explicitly disconnected the left list prior (explicit disconnect)
...
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # prev to be tail of reversed right half:
        prev = None

        # Set Curr:

        # Odd List Len: prev of first element of right list
        # setting curr to slow, which is saying mid
        # right is 1 element longer (because of mid element)

        # Even List Len: first element of right list
        # setting curr to slow, which is saying mid2
        # left and right are same length (split 1st and 2nd mid elements)
        curr = slow

        # Implicit Two Way Disconnect:
        # Both left and right lists have slow in their list
        # NOTICE: during the first modification of slow, we point it to null
        # now, both lists have a slow.next -> None

        # left still is pointing to slow and first element of right list
        # right will have slow as a last element with slow.next -> None
        ...
        # Now since:
        # Both left and right are referencing the same slow, with slow.next -> None
        # We know that when right == slow
        # the previous iteration is pointing to the left's reference of slow
        # so we can exit
        while right != slow:
            next_left, next_right = left.next, right.next
            left.next = right
...
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        while stack:
            ...
            # place right in between left and left.next
        ...
        # Implicit Disconnect Trick:
        # Even/Odd Len Case:
        # Regardless of difference in length between right nad left list,
        # since right.next = next_left,
        # this automatically connects the 1 or 2 extra nodes once
        # while right list stack terminates (since right list is shorted)
        # since we explicitly disconnected the left list prior (explicit disconnect)
...
### Solution 4: Stack - Linked List/Simple Traversal
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # Notice:
        # due to curr = slow: left list is smaller than right
        # therefore we need to terminate based on smaller left list
        # 
        ...
        # Odd/Even Case:
        # either right list is 1 element longer
        # or left and right are even length
        # upon termination left = right.next
        # so left is either a valid node, or None
        # if it is a valid node, set the left.next -> None
        # to terminate new list 
        if left:
            left.next = None
...
```python
    def removeNthFromEnd(self, head: Optional[ListNode], n: int) -> Optional[ListNode]:
        ...
        # 3. Traverse again to node just before the one to remove
        # 4. Adjust pointers to skip the nth node from the end
        ...
        # Second pass: find the node behind target
        dummy = ListNode(0, head)
        curr = dummy

        # stop at (len - n) - 1
        # -1 due to dummy, stops at node behind target
...
### Solution 1: Two Pass Hashmap - Linked List/Simple Traversal
```python
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        while curr:
            ...
            # grab deepCopy node (may be None, so use .get() in case)
...
### Solution 2: One Pass Memoization Hashmap with Lazy Construction - Linked List/Simple Traversal
```python
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        # Dummy node to simplify linking process
        dummy = Node(0)
        prev = dummy
        
        # orig node -> copied node
        memo = {}
        ...
        while head:
            ...
            # Step 1A: 
            # grab or create and grab current node
            if head not in memo:
                memo[head] = Node(head.val)
            copy_node = memo[head]
            
            # Step 1B: 
            # grab or create and grab random node
            if head.random:
                if head.random not in memo:
                    memo[head.random] = Node(head.random.val)
                copy_node.random = memo[head.random]
            
            # Step 1C: 
            # connect elements of new deep copy list during iteration,
            # dummy is now pointing to head of new deep copy list
            prev.next = copy_node

            # Step 1D: 
            # iterate to connect next node
...
```python
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        # 3. Detach copy list from original (restoring original)
        # 4. Constant space, but requires 3 passes 
        ...
        curr = head
        while curr:
            # grab and assign random to matching interweaved nodes
            if curr.random:
        ...
        while curr:
            ...
            curr.next = copy.next
            # point interweaved copy to new interweaved copy
            # A -> A' -> B -> B' (point A' -> B')
            if copy.next:
