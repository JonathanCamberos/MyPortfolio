...
### Solution 1: Slow.next Explicit Two Way Disconnect Iteration Reversal - Linked List/Simple Traversal
```python
...
### Solution 2: Slow One Way Disconnection Reversal - Linked List/Simple Traversal
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # Implicit disconnect:
        # Mid slow will point to None during reversal
        # both curr/slow reference same node in memory
        curr = slow
        ...
        # point last element to second to last element
        curr.next = prev

        # Notice: 
        # curr has extra element slow (should ony be on left half
        # but is on both left and right half)
        # Thus: to avoid double add slow: while curr != slow:

        # iterate over right list
        while curr != slow:
            left_next, right_next = head.next, curr.next
            head.next = curr
            curr.next = left_next
            head, curr = left_next, right_next
