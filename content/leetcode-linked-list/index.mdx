...
### Solution 1: Reversal and Merge - Linked List/Simple Traversal
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # Slow will point at middle
        slow, fast = head, head
        while fast and fast.next:
            slow = slow.next
            fast = fast.next.next

        # Prepare for iterative list reversal
        prev = None
        ...
        # by pointing last element from first half -> None
        # instead of -> head of second half        
        slow.next = None 
        ...
        # prev now points to start of reversed second half

        # alternating merge between left half and reversed right half
        left_half, right_half = head, prev

        # right half is always equal or shorter than the left half (by 1 node at most)
        while right_half:

            next_node1, next_node2 = left_half.next, right_half.next
            left_half.next = right_half
            right_half.next = next_node1
            left_half, right_half = next_node1, next_node2
...
### Solution 2: Advanced Reversal and Merge - Linked List/Simple Traversal
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # 2nd half list reversal (starting from 'slow')
        curr = slow
        prev = None
        while curr.next:
            tmp = curr.next
...
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # Note:
        # Store all nodes in a stack to allow reverse access
        # Iteratively link first node, last, second, second last, etc
        ...
        for _ in range(n // 2):
            ...
            # backwards iteration over stack
            tail = stack.pop()

            # split curr from curr.next
            tail.next = curr.next

            # chain tail between curr and curr.next
            curr.next = tail

            # iterate to curr.next
            curr = tail.next
