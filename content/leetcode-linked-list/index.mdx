...
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # Thus: Fully in place no extra space
        ...
        # Curr set to start of second half
        prev = None
        curr = slow.next

        # Disconnect last element from first half from second half        
        slow.next = None 

        # Reverse direction of second half of list
        while curr:
            next_node = curr.next
            curr.next = prev
            prev = curr
        ...
        # merge left half and right half alternating
        left_half, right_half = head, prev
        while right_half:

            next_node1 = left_half.next
            next_node2 = right_half.next
            left_half.next = right_half
            right_half.next = next_node1
            left_half = next_node1
            right_half = next_node2
