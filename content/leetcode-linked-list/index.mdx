...
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # Now since:
        # left equal or longer by 1 length by 1,
        # slow.next -> None is contained by both lists,
        # we will reach rights copy at the same time or earlier by 1 iteration
        # however, right will point to slows copy either way
        # so we can terminate 
        while right != slow:
            next_left, next_right = left.next, right.next
            left.next = right
...
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # Exit is based on smaller right list in both cases:
        ...
        # Explicit Disconnect: 
        # We set slow.next -> None
        # so left list is no longer pointing to right list,
        # thus once right list empty and
        # since right.next = next_left,
        # this automatically connects the remaining left list
        # (1 or 2 extra nodes depending if odd/even)
        # and slow.next -> None, terminates the left/merged list
...
```python
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # NOTICE:
        # No explicit disconnect (deal with that later)
