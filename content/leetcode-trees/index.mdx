...
### Tree Application: DFS In Order Traversal Mid Point
...
### Tree Application: DFS Post Order Traversal Bottom Up
...
```python
    def maxDepth(self, root: Optional[TreeNode]) -> int:
        # Note:
        # Idea: DFS recursive approach calculates depth from leaf upward
        # 2. Base case: if node is None, depth = 0
        # 3. Recursive case:
        #    depth_left = depth of left subtree
        #    depth_right = depth of right subtree
...
### Solution 4: DFS Iterative Stack - Tree/DFA Post Order Traversal
```python
    # Note:
...
### Solution 5: BFS Iterative - Tree/BFS Iterative Full Level Processing
```python
    def maxDepth(self, root: Optional[TreeNode]) -> int:
        # Note:
...
### Solution 6: BFS Iterative Node Depth Pair - Tree/BFS Iterative Full Level Processing
```python
    def maxDepth(self, root: Optional[TreeNode]) -> int:
        # Note:
        # 1. BFS without explicit level size
        # 2. Queue stores (node, depth)
        # 3. Track depth as you dequeue each node
