...
```python
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        ...
        # Note:
        # Pre order: root -> left -> root
        # In order: left -> root -> right
       
        # Note:
        # for in order, original root will be somewhere in middle, thus:
        # invariant:
        ...
        # for in order, iterating will decide which tree we are building
        pre_idx = 0
        ...
        def array_to_tree(left, right):
            ...
            # Create curr root node
            root = TreeNode(root_val)

            # 2: Find root's index in in order 
            #    to take advantage of invariant [leftSubtree, root, rightSubtree]
            root_index = inorder_index_map[root_val]

            # 3: given invariant, grab left subtree range for in order array
            left_subtree_left = left
            left_subtree_right = root_index - 1

            # 4: given invariant, grab right subtree range for in order array
...
```python
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        ...
        #    the algorithm iteratively processes the tree by 
        #    building subtrees one by one:
        root = TreeNode(preorder[0])

        # 2: stack:
        #    holds nodes whose subtrees are still being built
        #    top of stack, is the previous node waiting for its subtrees
        stack = [root]

        # 2: inorder_index:
        #    holds progress in inOrder array: left -> root -> right
        #    advances when node has received both children
        #    (only points to 'root', when 'root' receives left)

        # so we start at 0, since in order: left -> root -> right
        inorder_index = 0
        
        # 3: start 2nd element: root -> left -> right
        #.   since we start by processing left subtree
        for i in range(1, len(preorder)):

            # peek top of stack:
            # previous elem waiting for subtrees
            top = stack[-1]

            # current subtree of previous call (next root) to be added to tree
            preorder_val = preorder[i]
            
            # stack: root 
            # inorder: left -> root -> right

            # compare top of stack's value with inorder_val to decide if:
            #   1) top != inorder_val : preorder_val is a left child, connect and push to stack
            #   2) top == inorder_val -> preorder_val is a right child, connect and push to stack
            ...
            if top.val != inorder_val:
                ...
                # process -> left -> :
                # preorder_val is a left child, connect and push to stack        
                top.left = TreeNode(preorder_val)
                stack.append(top.left)
                        
            # finished processed left subtree
            # process -> right
            else:
                
                # keep pop() while initial ' if top.val != inorder_val: '
                # remains true: 
