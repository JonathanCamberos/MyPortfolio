...
```python
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        ...
        # Map each value to its in order index for O(1) lookups
        inorder_index_map = {val: idx for idx, val in enumerate(inorder)}

        # PreOrder index pointer to track root positions in preOrder list
        pre_idx = 0
        ...
        def array_to_tree(left, right):
            ...
            # 3: get root's left subtree range for inorder array
            left_subtree_left = left
            left_subtree_right = root_index - 1

            # 4: get root's right subtree range for inorder array
...
### Solution 2: Iterative Build Using Stack - Tree/DFS Pre order Traversal
```python
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
