...
Top Down: Root is processed immediately, 
then we do deep search in left or right subtree.
...
Traversal Order: Left -> Root -> Right  ||  Right -> Root -> Left 
Left Up or Right Up: Deep search on left or right subtree, 
root is processed, then deep search on right's left subtree 
or left's right subtree
(depending if first step was a deep search on left or right)
...
Traversal Order: Left -> Right -> Root  ||  Right -> Left -> Root 
Left Up then Right Up or Right Up then Left Up: Deep search on left or right subtree, root is processed, 
then deep search on right's left subtree or left's right subtree
(depending if first step was a deep search on left or right)
...
### Solution 1: DFS Post Order Recursive - Tree/DFS Post Order Traversal
```python
    def invertTree(root: Optional[TreeNode]) -> Optional[TreeNode]:
        # Note:
...
### Solution 2: DFS Pre Order Iterative Carry Down Tuple (Node, Depth) - Tree/DFA Pre Order Traversal
```python
    # Note:
    ...
    while stack:
        ...
        # process node: (node, prev depth)
        node, depth = stack.popleft()
        
        # check node maximum depth
