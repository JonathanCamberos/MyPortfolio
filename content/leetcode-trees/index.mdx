...
```python
    def invertTree(root: Optional[TreeNode]) -> Optional[TreeNode]:
        # Note:
        # 1. DFS recursive approach inverts subtrees from bottom up
        # 2. Base case: return None if node is null
        # 3. Swap left and right children at each node
        # 4. Recurse into left and right children
        # 5. Returns inverted root
...
```python
    def invertTree(self, root: Optional[TreeNode]) -> Optional[TreeNode]:
        ...
        while queue:
            ...
            # iterate over tree:
            # add curr root left and right children to queue,
            # curr root is no longer in the queue since we popped() already
            if node.left:
                queue.append(node.left)
            if node.right:
                queue.append(node.right)
...
## 104. Maximum Depth of Binary Tree ::2:: - Easy
...
### Solution 2: BFS Iterative - Tree/BFS Iterative Full Level Processing
```python
    def maxDepth(self, root: Optional[TreeNode]) -> int:
        # Note:
