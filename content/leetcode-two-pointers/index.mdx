...
We can use a single pointer to iterate linearly and keep track of
additional state variables to either simulate a second pointer
or store relevant data.

Ex: Move all zeros to end of array while maintaining order
```python
    def move_zeros(nums: list[int]) -> None:

        # Slot where non-zero value should go
        partition = 0

        # Pointer iteration
        for left in range(len(nums)):

            # Move non-zero values to current slot
            if nums[left] != 0:
                nums[last_non_zero_found_at], nums[left] = nums[left], nums[last_non_zero_found_at]
...
```python
    def is_palindrome(s: str) -> bool:
        ...
        # init to start and end of array
        left, right = 0, len(s) - 1

        # until middle of array is hit
        while left < right:

            # if palindrome is broken
            if s[left] != s[right]:
                return False

            # continue towards middle
            left += 1
            right -= 1
...
We can have two pointers represent a window from [Left, Right] over 
a sequence that expands or shrinks to satisfy a condition.

Ex: Find the length of the longest substring without repeating characters.
```python
    def longest_unique_substring(s: str) -> int:

        # start of window, left side
        left = 0

        # window data, stores unique chars within window range
        char_set = set()

        # window data, stores length of all windows
        maxLength = 0

        # expand window length, right side
        for right in range(len(s)):

            # if unique char is broken, remove from left side until
            # unique char is true again
            ...
            # update length, compare against global max
            maxLength = max(maxLength, right - left + 1)

        # global max window length      
        return maxLength
...
We can have have two pointers moving at different speeds to detect cycles 
...
```python
    ...
    # List with a cycle: 1 -> 2 -> 3 -> 4 -> 2 (cycle)
...
```python
    def partition(nums, pivot):
        ...
        for right in range(len(nums)):
            ...
            # condition: if curr val is less that pivot val, flip to left side
            if nums[right] < pivot:

                # swap curr val with flip slot
                nums[left], nums[right] = nums[right], nums[left]

                # iterate flip slot
                left += 1

        # left is first index where element >= pivot
        return nums, left 

    # partition([9, 3, 5, 2, 8, 1, 6], 5) -> [3, 2, 1, 5, 8, 9, 6], index = 5
```

## Two Pointers Application: K Parallel Pointer Traversal 
We can have k pointers traversing separate arrays in parallel to 
merge, compare, or find intersections.
...
```python
    def merge_sorted_arrays(arr1, arr2):
        ...
        # iterate both pointers, while elements remain in both lists 
        while i < len(arr1) and j < len(arr2):

            # compare both pointers, append smaller
            if arr1[i] < arr2[j]:
        ...
        # One list has run out of elements, append rest of list
...
remain frozen while the right pointer iterates until it hits a delimiter,
at which point the left jumps (catches up) to right+1 to mark the start
of the next section.
...
```python
    def split_words(s: str, delim: str = ' ') -> list[str]:
        ...
        # iterate looking for delimiters
        while right < len(s):

            # found delimiter
            if s[right] == delim:

                # non-empty word, splice and add to array
                if left != right:
                    words.append(s[left:right])

                # catch up: move start to after delim
                left = right + 1

            # move right pointer
            right += 1

        # hit end of string, check if last word exists
        if left < len(s):
            words.append(s[left:])
