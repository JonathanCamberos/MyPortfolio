...
### Solution 1: 2 Outer/2 Inner Pointers Creating Bounds Opposite Ends Pointer Shift by BinarySearch Modification (Sorting) - Two Pointers/K Pointer Variants
```python
    def trap(self, height: List[int]) -> int:
        ...
        while left < right:
            ...
            if height[left] < height[right]:
                ...
                # inner pointer taller than max, 
                # thus no wall/side to the left of inner left pointer 
                # so no water is able to be caught 
                # update max                
                if height[left] >= outerLeftMax:
                    outerLeftMax = height[left]

                # implies: height[left] < outerLeftMax < height[right]
                # bounded by outerLeftMax for water calculation
                # water is able to be caught
                else:
            ...
            # implies: height[right] < height[left]
                ...
                # inner pointer taller than max, 
                # thus no wall/side to the right of inner right pointer 
                # so no water is able to be caught 
                # update max      
                ...
                # bounded by outerRightMax for water calculation
                # water is able to be caught
                else:
...
```python
    def trap(self, height: List[int]) -> int:
        ...
        # Monotonic Stack: A stack that maintains indices of heights 
        # in a non-increasing order. As soon as a height greater than 
        # the height at the top of the stack is encountered, water trapping is calculated.
        ...
        for i in range(n):
            ...
            # Check: current height[i] can act as a right wall
            # height[i]: right wall
            # stack[-1]: depth candidate
            # implies: skipping and pushing right walls until a valid 
            # depth candidate is found
            # implies: only pop when it can serve as a depth candidate,
            # otherwise use as a left wall
            # implies: due to skipped and pushed right walls, 
            # stack is in monotonic non-increasing order
            while stack and height[i] > height[stack[-1]]:

                # implies: stack.pop() is valid depth candidate 
                depthCandidate = stack.pop() 

                # If the stack becomes empty after popping, no left wall exists
                # implies: cannot form a valid trap in this case
                if not stack:
                    break

                # After stack.pop():
                # stack[-1] is a new element
                # previous implication: stack is in non-increasing order
                # implies: any item on stack after pop can serve as left wall
                # implies: stack[-1] < depthCandidate < height[i]
                distance = i - stack[-1] - 1

                # Subtract the height of the bottom (depth candidate) to get water depth
                boundedHeight = min(height[i], height[stack[-1]]) - height[depthCandidate]

                # Add the trapped water for the current segment 
                water += distance * boundedHeight

            # index i is the position of the bar being processed in the array height
            # allows us to calculate distance between current bar and left wall
            stack.append(i)
