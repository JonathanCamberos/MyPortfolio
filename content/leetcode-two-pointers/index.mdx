...
```python 
    def twoSumII(self, numbers: List[int], target: int) -> List[int]:
        ...
        for i in range(len(numbers)): 
            ...
            # grab curr search item for BinarySearch
            comp = target - numbers[i]
            
            # given constraint index1 < index2, we can BinarySearch on the
            # right side section of the array after the left element
            # time complexity: binarySearch O(log n) for each iteration O(n), leading to O(n log n)
            left, right = i + 1, len(numbers) - 1
            
            # Base Case: no more array remaining to search, element not present
            while left <= right:
                ...
                # Recursively BinarySearch relevant left or right section
                elif midNum < comp:
                    left = mid + 1
                else:
                    right = mid - 1
...
### Solution 2: Shift Ends Based On BinarySearch Modification - Two Pointers/Opposite Ends
```python
    def twoSumII(self, numbers: List[int], target: int) -> List[int]:
        ...
        while left < right:
            ...
            # "BinarySearch" on relevant half
            # via increasing left or decreasing right pointer
            elif currSum < target:
                left += 1
            else:
                right -= 1
...
### Solution 1: Frozen Pointer with Two Pointers Early Break (Sorting) - Two Pointers/K Pointer Variants
```python
    def threeSum(self, nums: List[int]) -> List[List[int]]:
...
### Solution 1: Two Pointer Early Break - Two Pointers/Partitioning 
```python
    def maxArea(self, height: List[int]) -> int:
...
### Solution 2: Greedy - Two Pointers/Algorithm
...
### Solution 1: Two Pointers Early Break - Two Pointers/Opposite Ends
```python
    def trap(self, height: List[int]) -> int:
