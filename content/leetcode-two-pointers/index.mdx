...
```python
    def twoSumII(self, numbers: List[int], target: int) -> List[int]:
        ...
        while left < right:
            ...
            # Check Middle, If Left Path, Else Right Path

            # Check Middle: found target sum
            if currSum == target:
                # convert to 1-indexed array, per input/output example
                return [left + 1, right + 1]

            # Binary Search on relevant subsection:

            # Left Path: currSum is too low
            elif currSum < target:
                left += 1

            # Right Path: currSum is too high
            else:
                right -= 1
