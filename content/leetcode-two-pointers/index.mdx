...
image: "../../public/blogs/two-pointer.png"
...
| Inner loop | O(n<sup>2</sup>) | O(1) | For each element in the outer loop, the inner loop iterates through all subsequent elements with Big(O) is O(n<sup>2</sup>). Derivation [here](/blogs/common-formulas#brute-case-double-for-loop) | No additional memory allocation for iteration O(1) |
...
### Solution 1. Hashmap No Sorting (Two Sum I Approach)
```python    
    def threeSum(self, nums: List[int]) -> List[List[int]]:
...
### Solution 2: Two Pointers Sorting Early Break
```python
    def threeSum(self, nums: List[int]) -> List[List[int]]:
    ...
    def trap(self, height: List[int]) -> int:
        ...
        while L <= R:
            ...
            if height[L] < height[R]:
                ...
                # reverse implies right is taller, so add to water count
                else:
            ...
            # grabbing weak link, reverse implies left is taller so
            else:
                # Update rightMax
                if height[R] >= rightMax:
                    rightMax = height[R]

                # reverse implies left is taller, so add to water count
                else:
