...
### Solution 2: Shift Opposite Ends Based On BinarySearch Modification - Two Pointers/Opposite Ends
```python
    def twoSumII(self, numbers: List[int], target: int) -> List[int]:
        ...
        # Base Case: no more array remaining to search, return []
        # time complexity: iterate over list of n length O(n)
        while left < right:
