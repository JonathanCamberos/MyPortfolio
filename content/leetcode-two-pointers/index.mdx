...
```python
    def longestPalindrome(self, s: str) -> str:
        ...
        for i in range(1, n - 1):
            ...
            # initializing palindrome radius:
            # right: rightmost boundary of the palindrome centered at 'center'
            # i < right: means index i lies inside the palindrome that
            # extends from center to right
            if i < right:
                p[i] = min(right - i, p[mirror])

            # expanding around current center i while a valid palindrome exists
            while t[i + p[i] + 1] == t[i - p[i] - 1]:
                p[i] += 1

            # i: current center for substring
            # p[i]: radius of the longest palindrome centered at i
            # right: right edge index of palindrome that currently extends farthest to the right
            # center: center index of palindrome that currently extends farthest to the right
            if i + p[i] > right:
                # if for current center i + radius, extends farther than the previous
                # right boundary for previous longest palindromic substring, 
                # we have found a new longest palindromic substring, update center and right
                center = i
                right = i + p[i]
