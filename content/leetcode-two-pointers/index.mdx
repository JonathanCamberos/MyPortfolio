...
1. frozen negative number + mini twoSum
2. grouping by parity + 4 subcases

frozen negative number + mini twoSum:
...
### Solution 1: frozen negative number + mini twoSum
```python
...
### Solution 2: grouping by parity + 4 subcases
```python
def threeSum(self, nums: List[int]) -> List[List[int]]:
    p, n, z = [], [], []
    res = set()
