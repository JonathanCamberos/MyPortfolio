...
## Solution 1: Monotonic 2 Inner/Outer Pointers Traversal Creating Bound Buckets Opposite Ends Pointer Shift Modification - Two Pointers/K Pointer Variants
```python
    def trap(self, height: List[int]) -> int:

        # Bound Buckets:
        # [4, 0, 2, 6, 3, 5]: monotonic quality determine buckets: (bucket from 4 -> 6, bucket from 6 -> 5)
        #                     which are monotonic increasing and monotonic increasing (in reverse iteration) buckets
        #    if we use 4 and 6 as the bucket boundaries, the bucket will catch any water up to and including 4
        #    if we use 6 and 5 as the bucket boundaries, the bucket will catch any water up to and including 5
        ...
        # Creating Buckets: 
        # Outer Pointers: act as wall/side to outline bucket so inner depth pointers can verify monotonic quality
        # Inner Pointers: traverse inward from both ends to track depth/height for current position
        # Water Trapping: compare inner to outer pointer height determine if theres enough depth to trap water
        ...
        # Monotonic
        # tc: two pointer iteration over list of n length O(n)
        while left < right:

            # case 1: [5, 0, 3, 2, 4, 6], left < right for entire array (bucket from 5 -> 6)
            # case 2: [5, 0, 3, 9, 4, 6], left < right broken at some point (bucket from 5 -> 9, 9 -> 6)
            ...
            # Thus: water is limited by outerLeft/outerRight because it makes a bucket 
            # as water will eventually be caught by right because of the implication
            # height[left] < outerLeftMax < outerRightMax
            # so water caught is now determined by outerLeftMax - height[left]

            # on first iteration, this also checks if outerLeftMax < outerRightMax
            # because we set outerLeft = height[left]
            # so we know height[left] is at least 1 less than outerLeftMax
            # and know   height[right] is at least 1 less than outerRightMax
            if height[left] < height[right]:

                # case 1: 
                # implication (height[left] < outerLeftMax < outerRightMax) broken
                # implies: outerLeftMax < height[left] ? outerRightMax (checked next iteration)
                # then: update outerLeftMax to height[left]           
                if height[left] >= outerLeftMax:
                    outerLeftMax = height[left]

                # case 1: implication (height[left] < outerLeftMax < outerRightMax) applies
                # implies: height[left] < outerLeftMax < height[right]
                # then: check if water has been caught, determined by outerLeftMax - height[left]
                else:
                ...
                # case 1: implication (height[right] < outerRightMax < outerLeftMax) broken
                # implies: outerRightMax < height[right] ? height[left] (checked next iteration)   
                # then: update outerRightMax to height[right]
                if height[right] >= outerRightMax:
                    outerRightMax = height[right]

                # case 1: implication (height[right] < outerRightMax < outerLeftMax) applies
                # implies: height[right] < outerRightMax < outerLeftMax
                # then: check if water has been caught, determined by outerRightMax - height[right]
                else:
...
## Solution 2: Monotonic Stack Dragging Right Wall Height Over The Array And Catching Water With Depth Candidates And Left Wall - Two Pointers/Algorithm
```python
    def trap(self, height: List[int]) -> int:
...
## Solution 3: Dynamic Programming Creating Bucket By Tracking Max Height Bucket Bounds Encountered L To R and R to L - Two Pointers/Algorithm
```python
    def trap(self, height: List[int]) -> int:
        ...
        # Arrays to store max heights from the left and right sides
        # leftMax[i]: Maximum height from 0 to index i (via iterating left to right)
        # rightMax[i]: Maximum height from index i to n-1 (via iterating right to left)
        # sc: relative to input O(n)
        leftMax = [0] * n
        rightMax = [0] * n
        water = 0
        ...
        # Left Max Array:
        # Max between left previous max and current height
        # tc: iterate over n O(n)
        leftMax[0] = height[0]
        for i in range(1, n):
            leftMax[i] = max(leftMax[i - 1], height[i])
        ...
        # Right Max Array:
        # Max between right previous max and current height
        # tc: iterate over n O(n)
        rightMax[n - 1] = height[n - 1]
        for i in range(n - 2, -1, -1):
            rightMax[i] = max(rightMax[i + 1], height[i])
        ...
        # Depth Calculation:
        # Check if bounded by left or right max (lower of the two) and check if there is depth to catch water
        # tc: iterate over n O(n)
        for i in range(n):

            # Water trapped at bar i is lower bound between left and right max
            # minus the curr bar height
