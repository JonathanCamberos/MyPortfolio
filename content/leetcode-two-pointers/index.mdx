...
```python
    def longestPalindrome(self, s: str) -> str:
        ...
        # that expands the farthest to the right in the preprocessed string
        center = 0 

        # right: right boundary of the rightmost palindrome found so far
        # that expands the farthest to the right in the preprocessed string
        right = 0
        ...
        for i in range(1, n - 1):
            ...
            # for some palindrome centered at 'center'
            # there exists a mirror position on the opposite side of 'center'
            # this mirror is the symmetric counterpart of i
            mirror = 2 * center - i 

            # checking if the current center, list within the
            # current longest palindromic substring
            if i < right:
                
                # we know that:
                # curr longest palindrome is centered at 'center'
                # curr longest palindrome extends to 'right'
                # i is within this palindromes boundaries
                # implies: palindrome centered at i, as long as minimum between:
                # - palindrome radius at mirror (the symmetric position around the current center)
                # - distance from i to right (because palindrome centered at i can't extend beyond current longest palindromic substring)
                p[i] = min(right - i, p[mirror])
