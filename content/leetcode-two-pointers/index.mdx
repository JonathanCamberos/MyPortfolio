...
```python 
    def twoSumII(self, numbers: List[int], target: int) -> List[int]:
        ...
        for i in range(len(numbers)): 
            ...
            # 3 Paths: Middle Target, If Left Path, Else Right Path

            # base case: no more array to search, numbers[i] complement
            # was not found, check next iteration
            while left <= right:
                ...
                # check left path: currSum is too low
...
```python
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        for i in range(n - 2):
            ...
            # thus: left < right instead of left <= right
            ...
            while left < right:
                ...
                if currSum == 0:
                    ...
                    # ignore duplicates complete pair:
                    # we already found j and k, we can skip duplicates of j or k
                    while left < right and nums[left] == nums[left - 1]:
                        left += 1
                    while left < right and nums[right] == nums[right + 1]:
                        right -= 1
...
```python
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        # group by: positive, negative, and zero sets
        # sc: relative to input O(n)
        p, n, z = [], [], []
        ...
        # (0, num, -num): at least 1 additive inverse for a valid triplet with at least 1 0
        if len(z) > 0:
            # tc: iterate n positive numbers O(n)
            for i in P:                
                nTarget = -i
                # tc: additive inverse target lookup constant O(1)
                if nTarget in N:
                    # tc: put constant O(1)
                    res.add((nTarget, 0, i ))
        ...
        for i in range(len(n)):
            ...
            for j in range(i+1, len(n)):
                pTarget = -(n[i] + n[j])
                # tc: additive inverse lookup constant O(1)
                if pTarget in P:
                    # tc: put constant O(1)
...
## Solution 1: Greedy Opposite Ends Pointer Shift by BinarySearch Modification [TC Opt] - Two Pointers/Opposite Ends 
```python
    def maxArea(self, height: List[int]) -> int:
