---
title: "LeetCode: I Graphs DFS BFS Union Find"
description: "graphs"
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                if (grid[r][c] == '1' and 
                    ...
                    # zero out this entire island's land
                    dfs_MarkVisitedLand(r, c)
...
## Solution 2: [DFS] DFS Recursive Early Pruning Sink All Visited Land In Current Island - Graph/something
```python
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        # visits any land touching (r, c)
        # sc: O(r*c) worst case recursion depth
        def dfs_SinkVisitedLand(r, c):
...
## Solution 3: [BFS] BFS Iterative Early Pruning Sink All Visited Land In Current Island - Graph/something
```python
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        # Return:
        # implicit, queue empties after all land in current island is explored
...
```python 
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        # Rank:                     After: Find(3) and path compression
        #   0
        #   |                           0           
        #   1       Rank of          |  |  |        But rank is
        #   |           3            1  2  3          still 3 
        #   2
        #   |                                 - Recomputing heights would be expensive
        #   3                                 - Rank only guides future unions
        #                                     - Rank is accurate before compression, not after compression
        #                                     - Rank = upper bound on tree height
        #
        # Rank represents a conservative estimate of tree height used for balancing. 
        # Path compression can flatten trees and reduce real height, 
        # but rank is not updated because it only needs to remain an upper bound.

        # Rank Fun Fact:
        # Rank grows exponentially with number of nodes
        # For rank of 30 we need 2^30 = ~1 billion nodes required
        # And since most problems are << 1 billion, we never pass a rank of 30
        # which usually ensures a time complexity of O(1)

        # Path Compression
        # Path compression flattens the trees whenever you call find(),
        # It keeps trees balanced and prevents extremely tall trees
        # After a few finds, the tree can become almost flat
        # It doesn't prevent the rank from being high, but it
        # dramatically reduces the actual traversal length during finds

        # Path Compression + Union By Rank:
        # Actual Tree Height << log2(N)
        # Which allows us to say the amortized cost per union/find is α(N)
        # α(N) grows so slowly that for all practical N (even N ~ 10^80). its <= 5)
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                if grid[r][c] == '1':
                    ...
                    # Each land cell is its own root land for its island initially
                    parent[(r, c)] = (r, c)
        ...
        def union(x, y):
            ...
            if rank[rootX] > rank[rootY]:
               parent[rootY] = rootX
                ...
                # Both Trees Have Same Rank:
                # Join to either, and increase rank of parent by 1 
                parent[rootY] = rootX
                rank[rootX] += 1
...
Same as Number of Islands except we just keep track of the size of the current 
island as we sink it.
